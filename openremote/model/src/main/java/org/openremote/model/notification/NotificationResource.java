...
//                             @QueryParam("assetId") List<String> assetIds);
    ...
    @Produces(APPLICATION_JSON)
    @Path("getData")
    List<NotificationDTO> getAll(SearchFilterDTO<SentNotification> filterDTO);

    @POST
    @Path("countData")
    @Consumes(APPLICATION_JSON)
    @Produces(APPLICATION_JSON)
    Long countData(SearchFilterDTO<SentNotification> filterDTO);

    @POST
    @Consumes(APPLICATION_JSON)
    @Produces(APPLICATION_JSON)
    @Path("getData/{userId}")
    List<SentNotification> getSentNotifications(@PathParam("userId") String userId);

    @PUT
    @Path("{notificationId}/markAsRead")
    void markAsRead(@BeanParam RequestParams requestParams,
                               @PathParam("notificationId") Long notificationId);
