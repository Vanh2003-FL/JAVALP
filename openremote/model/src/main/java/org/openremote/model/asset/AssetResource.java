...
public interface AssetResource {
    ...
    @POST
    @Path("reloadBUIE/{assetId}")
    void reloadBUIE(@BeanParam RequestParams requestParams,
                             @PathParam("assetId") String assetId);
    ...
    @Produces(APPLICATION_JSON)
    @RolesAllowed({Constants.READ_ASSETS_ROLE})
    List<AssetTypeDto> getAllAssetType(@BeanParam RequestParams requestParams);

    @GET
    @Path("{assetId}")
    @Produces(APPLICATION_JSON)
    @RolesAllowed({Constants.READ_ASSETS_ROLE})
    AssetInfoDto get(@BeanParam RequestParams requestParams, @PathParam("assetId") String assetId);

    @GET
    @Path("getAsset/{assetId}")
    @Produces(APPLICATION_JSON)
    List<AssetInfoDto>  getAsset(@BeanParam RequestParams requestParams, @PathParam("assetId") String assetId);

    @GET
    @Path("getInfoLightAsset/{assetId}")
    @Produces(APPLICATION_JSON)
    LampTypeAssetDTO getInfoLightAsset(@BeanParam RequestParams requestParams, @PathParam("assetId") String assetId);
    ...
    @Produces(APPLICATION_JSON)
    @RolesAllowed({Constants.WRITE_ASSETS_ROLE})
    AssetInfoDto update(@BeanParam RequestParams requestParams, @PathParam("assetId") String assetId, AssetInfoDto assetInfoDto, @QueryParam("childIds") List<String> childIds);
    ...
    @PUT
    @Path("writeAttribute/{assetId}/attribute/{attributeName}")
    @Consumes(APPLICATION_JSON)
    @Produces(APPLICATION_JSON)
    @Operation(description = "Write to a single attribute", responses = {
            @ApiResponse(
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = AttributeWriteResult.class)))})
    Response writeAttributeValueExtra(@BeanParam RequestParams requestParams, @PathParam("assetId") String assetId, @PathParam("attributeName") String attributeName, Object value, @QueryParam("fixGroupId") String fixGroupId);
    ...
    @Produces(APPLICATION_JSON)
    @RolesAllowed({Constants.WRITE_ASSETS_ROLE})
    AssetInfoDto create(@BeanParam RequestParams requestParams, AssetInfoDto assetInfoDto);

    @POST
    @Consumes(APPLICATION_JSON)
    @Produces(APPLICATION_JSON)
    @Path("createGroupCabinet")
    AssetInfoDto createGroupCabinet(@BeanParam RequestParams requestParams, AssetInfoDto assetInfoDto);
    ...
    @Produces(APPLICATION_JSON)
    @RolesAllowed({Constants.WRITE_ASSETS_ROLE})
    void createRoute(@BeanParam RequestParams requestParams, RouteInfoCreateDTO routeInfoCreateDTO,@QueryParam("realm")String realm);
    ...
    @Produces(APPLICATION_JSON)
    @RolesAllowed({Constants.WRITE_ASSETS_ROLE})
    void updateRoute(@BeanParam RequestParams requestParams, RouteInfoCreateDTO routeInfoCreateDTO);
    ...
    @Produces(APPLICATION_JSON)
    @RolesAllowed({Constants.READ_ASSETS_ROLE})
    List<RouterAssetCreate> getUpdateRoute(@BeanParam RequestParams requestParams, @QueryParam("routeId") String routeId);
    ...
    @Consumes(APPLICATION_JSON)
    @Produces(APPLICATION_JSON)
    AssetInfoDto[] queryAssets(@BeanParam RequestParams requestParams, AssetQuery query);

    @POST
    @Path("queryMap")
    @Consumes(APPLICATION_JSON)
    @Produces(APPLICATION_JSON)
    AssetInfoDto[] queryMapAssets(@BeanParam RequestParams requestParams, AssetQuery query);

    @GET
    @Path("findDisconnectLights")
    @Consumes(APPLICATION_JSON)
    @Produces(APPLICATION_JSON)
    AssetInfoDtoWrapper findDisconnectLights(@QueryParam("realm") String realm);

    @POST
    @Path("queryLastTimeActive/{id}")
    @Consumes(APPLICATION_JSON)
    @Produces(APPLICATION_JSON)
    Date queryLastTimeActive(@BeanParam RequestParams requestParams, @PathParam("id") String id);

    @POST
    @Path("queryRoutes/{route_id}")
    @Consumes(APPLICATION_JSON)
    @Produces(APPLICATION_JSON)
    AssetInfoDto[] queryTreeRoutes(@BeanParam RequestParams requestParams, @PathParam("route_id") String routeId, @QueryParam("realm") String realm, @QueryParam("keyWord") String keyWord);
    ...
    void updateParent(
            ...
            @QueryParam("childIds") List<String> childIds
    ...
    @Produces(APPLICATION_JSON)
    @Path("/all-light")
    public List<FilterDto> getAllLightAssets(@BeanParam RequestParams requestParams);
    @GET
    @Produces(APPLICATION_JSON)
    @Path("/all-light-cabinet/{realm}")
    public List<FilterDto> getAssetName(@BeanParam RequestParams requestParams, @PathParam("realm") String realm, @QueryParam("types") String ... type);

    @GET
    @Path("getInfoLightByLightId")
    @Consumes(APPLICATION_JSON)
    @Produces(APPLICATION_JSON)
    LightInfoDTO getInfoLightByLightId(@BeanParam RequestParams requestParams, @QueryParam("lightId") String lightId);

    @POST
    @Path("createCabinetAsset")
    @Consumes(APPLICATION_JSON)
    @Produces(APPLICATION_JSON)
    @RolesAllowed({Constants.WRITE_ASSETS_ROLE})
    CabinetAssetDTO createCabinetAsset(CabinetAssetDTO cabinetAsset);
}
