...
import org.openremote.model.dto.NotificationDTO;
...
import org.openremote.model.routeInfo.RouteInfoDTO;
...
import static org.openremote.model.notification.Notification.HEADER_SOURCE_ID;
...
public class NotificationService extends RouteBuilder implements ContainerService {
                                ...
                                Exception notificationError = persistenceService.doReturningTransaction(em -> {
                                    ...
                                    if (notification.realm == null) {
                                        sentNotification = new SentNotification()
                                                ...
                                                .setSentOn(Date.from(timerService.getNow())).setIsRead(false);
                                    } else {
                                        sentNotification = new SentNotification()
                                                .setName(notification.getName())
                                                .setType(notification.getMessage().getType())
                                                .setSource(WARNING)
                                                .setSourceId(notification.realm)
                                                .setTarget(target.getType())
                                                .setTargetId(target.getId())
                                                .setMessage(notification.getMessage())
                                                .setSentOn(Date.from(timerService.getNow())).setIsRead(false);
    ...
    public void markAsRead(long id) {
        persistenceService.doTransaction(entityManager -> {
            Query query = entityManager.createQuery("UPDATE SentNotification SET isRead= TRUE WHERE id =:id");
            query.setParameter("id", id);
            query.executeUpdate();
        });
    }
    ...
    public List<SentNotification> getSentNotifications(String userId) {
        StringBuilder builder = new StringBuilder();
        builder.append("select n from SentNotification n ");
        builder.append("JOIN UserAssetLink ual ON n.targetId = ual.id.assetId ");
        builder.append("WHERE ual.id.userId = :userId and n.error is null and n.type = 'push' ");
    ...
    public List<NotificationDTO> getAll(SearchFilterDTO<SentNotification> searchFilterDTO) {
        return persistenceService.doReturningTransaction(em -> {
            String baseQuery = "SELECT " +
                    "n.message ->> 'subject' AS titleWarning, " +
                    "n.message ->> 'html' AS htmlWarning, " +
                    "substring(n.target_id FROM 'to:([^,\\s]+)') AS emailCustomer, " +
                    "n.name AS attributeName, " +
                    "n.source_id AS realm, " +
                    "n.sent_on AS timeSent " +
                    "FROM notification n " +
                    "inner JOIN public.realm r ON r.name = n.source_id " +
                    "INNER JOIN public.realm_attribute ra ON ra.realm_id = r.id AND ra.name = 'displayName' " +
                    "WHERE n.source = 'WARNING' AND n.type = 'email' ";

            if (validationUtils.isValid(searchFilterDTO.getKeyWord())) {
                baseQuery += " AND (LOWER(n.message ->> 'subject') LIKE LOWER(:KeyWord) OR " +
                        "LOWER(substring(n.target_id FROM 'to:([^,\\s]+)')) LIKE LOWER(:KeyWord) OR " +
                        "LOWER(n.name) LIKE LOWER(:KeyWord))";
            }

            if (validationUtils.isValid(searchFilterDTO.getData())) {
                if (validationUtils.isValid(searchFilterDTO.getData().getSourceId())) {
                    baseQuery += " AND n.source_id = :Realm";
                }
            }
            baseQuery += " order by n.sent_on desc ";
            ...
            if (validationUtils.isValid(searchFilterDTO.getData())) {
                if (validationUtils.isValid(searchFilterDTO.getData().getSourceId())) {
                    query.setParameter("Realm", searchFilterDTO.getData().getSourceId());
                }
            }
            ...
            log.info("SQL = {}", nativeQuery.getQueryString());
            ...
            for (Object[] row : rows) {
                ...
                dto.setTitleWarning(row[0] != null ? (String) row[0] : "");
                dto.setHtmlWarning(row[1] != null ? (String) row[1] : "");
                dto.setEmailCustomer(row[2] != null ? (String) row[2] : "");
                dto.setAttributeName(row[3] != null ? (String) row[3] : "");
                dto.setRealm(row[4] != null ? (String) row[4] : "");
                dto.setTimeSent(row[5] != null ? convertToDate(row[5]) : new Date(0));

                notificationDTOList.add(dto);
            }

            notificationDTOList.sort(Comparator.comparing(NotificationDTO::getTimeSent, Comparator.nullsLast(Comparator.reverseOrder())));

            return notificationDTOList;
        });
    }

    public Long count(SearchFilterDTO<SentNotification> filterDTO) {
        return persistenceService.doReturningTransaction(em -> {
            SentNotification notification = filterDTO.getData();
            String baseQuery = """
                        SELECT COUNT(*)
                        FROM notification n
                        RIGHT JOIN public.realm r ON r.name = n.source_id 
                        INNER JOIN public.realm_attribute ra ON ra.realm_id = r.id AND ra.name = 'displayName' 
                        WHERE n.source = 'WARNING' 
                    """;
            ...
            if (validationUtils.isValid(notification)) {
                if (validationUtils.isValid(filterDTO.getData().getSourceId())) {
                    baseQuery += " AND n.source_id = :realm";
                }
...
//                }
            ...
            if (validationUtils.isValid(notification)) {
                if (validationUtils.isValid(filterDTO.getData().getSourceId())) {
                    query.setParameter("realm", filterDTO.getData().getSourceId());
                }
//                if (validationUtils.isValid(routeInfo.getStatus())) {
