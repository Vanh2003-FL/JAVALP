...
public interface ManagerIdentityProvider extends IdentityProvider {
    ...
    User updateUserExtend(User user);
    ...
    Realm[] getRealmAll(String realm);
    ...
    static User[] getUsersFromDb(PersistenceService persistenceService, UserQuery query) {
        ...
        if (userQuery.realmPredicate != null && (!TextUtil.isNullOrEmpty(userQuery.realmPredicate.name)||!TextUtil.isNullOrEmpty(userQuery.realmPredicate.id))) {
            sb.append(" AND (r.name = ?").append(parameters.size() + 1);
            parameters.add(userQuery.realmPredicate.name);

            sb.append(" OR r.id = ?").append(parameters.size() + 1).append(")");
            parameters.add(userQuery.realmPredicate.id);
        }
    ...
    @SuppressWarnings("unchecked")
    static Realm[] getRealmsFromDb(PersistenceService persistenceService, String realm) {
        StringBuilder baseSQL = new StringBuilder("SELECT *, \n" +
                "       (SELECT ra.value \n" +
                "        FROM public.realm_attribute ra \n" +
                "        WHERE ra.realm_id = r.id AND ra.name = 'displayName') AS displayName\n" +
                "FROM public.realm r\n" +
                "WHERE (r.not_before IS NULL OR r.not_before = 0 OR r.not_before <= EXTRACT(EPOCH FROM now())) ");
        if (realm != null) {
            baseSQL.append(" AND r.name = ?");
        }
        return persistenceService.doReturningTransaction(entityManager -> {
            List<Realm> realms;
            if (realm != null) {
                realms = (List<Realm>) entityManager.createNativeQuery(
                                baseSQL.toString(),
                                Realm.class)
