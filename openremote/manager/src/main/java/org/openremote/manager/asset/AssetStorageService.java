...
import org.openremote.model.dto.FilterDto;
import org.openremote.model.dto.LightInfoDTO;
import org.openremote.model.hdi.hdiDTO.*;
import org.openremote.model.dto.AssetInfoDto;
...
public class AssetStorageService extends RouteBuilder implements ContainerService {
    ...
    public void updatePathChilds(List<String> childIds) {
        persistenceService.doTransaction(em -> {
            String sql = "UPDATE ASSET " +
                    "SET VERSION = VERSION + 1 " + // Câu lệnh này không có tác dụng, có thể bỏ đi nếu không cần thiết
                    "WHERE id IN (:ids)";
            Query query = em.createNativeQuery(sql);
            query.setParameter("ids", childIds); // Đặt tham số danh sách đúng cách
            query.executeUpdate();
        });
    }

    public List<CabinetMetricsDTO> getCabinetMetricsByParentid(String fromDate, String toDate, String roadId, String realm) {
        return persistenceService.doReturningTransaction(entityManager -> {
            ...
            // Gọi stored procedure có thêm tham số realm
            String sql = "SELECT * FROM get_cabinet_metrics_by_parentid(:roadId, :startTime, :endTime, :realm)";

            Query query = entityManager.createNativeQuery(sql, CabinetMetricsDTO.class);
            query.setParameter("roadId", roadId.isEmpty() ? null : roadId);
            query.setParameter("startTime", Timestamp.valueOf(startTime.format(outputFormatter)));
            query.setParameter("endTime", Timestamp.valueOf(endTime.format(outputFormatter)));
    ...
    public List<CabinetMetricsDTO> getCabinetMetricsByParentid2(String fromDate, String toDate, String cabinetId, String realm) {
        return persistenceService.doReturningTransaction(entityManager -> {
            // Chuyển đổi chuỗi ngày thành LocalDateTime
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
            DateTimeFormatter outputFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
            ...
            // Gọi stored procedure có thêm tham số realm
            String sql = "SELECT * FROM get_cabinet_metrics_by_parentid_2(:cabinetId, :startTime, :endTime, :realm)";

            Query query = entityManager.createNativeQuery(sql, CabinetMetricsDTO.class);
            query.setParameter("cabinetId", cabinetId.isEmpty() ? null : cabinetId);
            query.setParameter("startTime", Timestamp.valueOf(startTime.format(outputFormatter)));
            query.setParameter("endTime", Timestamp.valueOf(endTime.format(outputFormatter)));
            query.setParameter("realm", realm);
    ...
    public void updateStartAssetInfo(String id, String status) {
        persistenceService.doTransaction(em -> {;
            String sql = "UPDATE asset_info " +
                    "SET status = :status " +
                    "WHERE id = :id";
            Query query = em.createNativeQuery(sql);
...
//    public List<AssetInfoEntity> findAllAssetInfo(List<String> lightCabinetIds) {
//         return persistenceService.doReturningTransaction(entityManager -> {
//            String sql = "SELECT new AssetInfoEntity (ai.id,ai.assetCode,ai.createDate,ai.createBy,ai.updateDate,ai.updateBy,ai.provinceId,ai.districtId,ai.wardId,ai.streetName,ai.address,ai.status,ai.supplierId,ai.firmwareVersion,ai.assetModel,ai.serialNumber,ai.lastMainceDate,ai.nextMainceDate,ai.deleted,ai.description,ai.lastTimeWarning,ai.lastValueWarning) FROM AssetInfoEntity as ai WHERE id IN (:ids)";
//            Query query = entityManager.createQuery(sql, AssetInfoEntity.class);
//            query.setParameter("ids", lightCabinetIds);
//            return query.getResultList();
//        });
//    }
    ...
    public List<AssetInfoEntity> findAllAssetInfo(List<String> lightCabinetIds) {
         return persistenceService.doReturningTransaction(entityManager -> {
             String sql = "SELECT id, asset_code, create_date, create_by, update_date, update_by, province_id, district_id, ward_id, " +
                     "street_name, address, status, supplier_id, firmware_version, asset_model, serial_number, last_maince_date, " +
                     "next_maince_date, deleted, description, last_time_warning, last_value_warning " +
                     "FROM asset_info WHERE id IN (:ids)";
             ...
             return results.stream().map(row -> {
                 ...
                 Timestamp createDateTS = (Timestamp) row[2];
                 LocalDateTime createDate = createDateTS != null ? createDateTS.toLocalDateTime() : null;

                 String createBy = (String) row[3];

                 Timestamp updateDateTS = (Timestamp) row[4];
                 LocalDateTime updateDate = updateDateTS != null ? updateDateTS.toLocalDateTime() : null;

                 String updateBy = (String) row[5];

                 Integer provinceId = (Integer) row[6];
                 Integer districtId = (Integer) row[7];
                 Short wardId = (Short) row[8];

                 String streetName = (String) row[9];
                 String address = (String) row[10];
                 String status = (String) row[11];
                 Integer supplierId = (Integer) row[12];
                 String firmwareVersion = (String) row[13];
                 String assetModel = (String) row[14];
                 String serialNumber = (String) row[15];

                 Timestamp lastMainceDateTS = (Timestamp) row[16];
                 LocalDateTime lastMainceDate = lastMainceDateTS != null ? lastMainceDateTS.toLocalDateTime() : null;

                 Timestamp nextMainceDateTS = (Timestamp) row[17];
                 LocalDateTime nextMainceDate = nextMainceDateTS != null ? nextMainceDateTS.toLocalDateTime() : null;

                 Boolean deleted = (Boolean) row[18];
                 String description = (String) row[19];

                 // last_time_warning là ZonedDateTime, thường trong DB sẽ là timestamp hoặc string
                 // Bạn cần kiểm tra kiểu thật của row[20]
                 ZonedDateTime lastTimeWarning = null;
                 Object lastTimeWarningObj = row[20];
                 if (lastTimeWarningObj instanceof Timestamp ts) {
                     lastTimeWarning = ts.toInstant().atZone(ZoneId.systemDefault());
                 } else if (lastTimeWarningObj instanceof String str) {
                     lastTimeWarning = ZonedDateTime.parse(str);
                 }

                 String lastValueWarning = (String) row[21];

                 return new AssetInfoEntity(id, assetCode, createDate, createBy, updateDate, updateBy,
                         provinceId, districtId, wardId, streetName, address, status,
                         supplierId, firmwareVersion, assetModel, serialNumber,
                         lastMainceDate, nextMainceDate, deleted, description, lastTimeWarning, lastValueWarning);
             }).toList();
...
//    }
    ...
    protected AssetInfoPersistenceService assetInfoPersistenceService;
    ...
    public void init(Container container) throws Exception {
        ...
        assetInfoPersistenceService = container.getService(AssetInfoPersistenceService.class);

        cabinetGroupPersistenceService = container.getService(CabinetGroupPersistenceService.class);

        cabinetAssetPersistenceService = container.getService(CabinetAssetPersistenceService.class);
...
//        AssetInfoPersistenceService assetInfoPersistenceService = container.getService(AssetInfoPersistenceService.class);
//        if (assetInfoPersistenceService == null) {
//            throw new IllegalStateException("AssetInfoPersistenceService not found in container");
//        }
        ...
        container.getService(ManagerWebService.class).addApiSingleton(
                new AssetResourceImpl(
                        ...
                        clientEventService,
                        assetInfoPersistenceService,
                        container.getService(SysAssetTypePersistenceService.class),
                        container.getService(AssetDatapointService.class),
                        cabinetGroupPersistenceService,
                        cabinetAssetPersistenceService
                )
        );
                ...
                .process(exchange -> {
                    ...
                    if (hasAssetQuery instanceof ReadAssetsEvent) {
                        ...
                        for (Asset<?> asset : assets) {
                            ...
                            if (assetInfo == null) {
                                assetInfoDtos.add(new AssetInfoDto(asset, new Asset_Info()));
                            }else assetInfoDtos.add(new AssetInfoDto(asset, assetInfo));
                        }
                        response = new AssetsEvent(assets, assetInfoDtos);
                    } else {
                        ...
                        if (asset != null) {
                            ...
                            if (!TextUtil.isNullOrEmpty(attributeName)) {
                                ...
                                Asset_Info assetInfo = assetInfoPersistenceService.getById(asset.getId());
                                AssetInfoDto assetInfoDto = new AssetInfoDto(asset, assetInfo);
                                response = new AssetEvent(AssetEvent.Cause.READ, asset, null, assetInfoDto);
                            }
                        }
                    }
...
//            }
            ...
            asset.getAttributes().forEach(attribute -> {
                if (attribute.getMeta() == null) {
                    attribute.setMeta(new MetaMap()); // hoặc new HashMap<>()
                }
    ...
    public boolean deleteAssetCabinet(List<String> assetIds) {
        try {
            persistenceService.doTransaction(em -> {
                em.createNativeQuery("DELETE FROM asset_cabinet WHERE asset_id IN (:assetIds)")
    ...
    public boolean deleteAssetInfo(List<String> assetIds) {
        try {
            persistenceService.doTransaction(em -> {
                em.createNativeQuery("DELETE FROM asset_info WHERE id IN (:assetIds)")
                        .setParameter("assetIds", assetIds)
                        .executeUpdate();
            });
        } catch (Exception e) {
    ...
    public boolean isCabinetGroupPowerOn(String assetId) {
        return persistenceService.doReturningTransaction(em -> {
            List<String> result = em.createNativeQuery("""
            SELECT a.attributes -> cg.cabinet_grp_name ->> 'value' AS power_value
            FROM asset_cabinet ac
            LEFT JOIN asset a ON a.id = ac.cabinet_id
            LEFT JOIN asset_cabinet_group acg ON acg.asset_id = ac.asset_id
            LEFT JOIN sys_cabinet_group cg ON cg.cabinet_grp_id = acg.cabinet_grp_id
        ...
        """)
            ...
            return "1".equals(result.get(0)); // chỉ trả true nếu giá trị bằng "1"
        });
    }
...
//        });
    ...
    //viet hdi begin
    // Xây dựng phần truy vấn động


    public void reloadBUIE(String assetId) {
        persistenceService.doTransaction(em -> {
            Map<String, List<LightCommand3Control2>> groupedResults = usrSpGetAssetInfoListByParentId(em, assetId);
            groupedResults.forEach((key, valueV) -> {
                HdiEven hdiEven = new HdiEven(new Hdi4State());
    ...
    public Map<String, List<LightCommand3Control2>> usrSpGetAssetInfoListByParentId(EntityManager em, String parentId) {
        String sql = "SELECT * FROM usr_sp_getAsset_info_list_by_parentId(:parentId)";

        Query query = em.createNativeQuery(sql, LightCommand3Control2.class);

        query.setParameter("parentId", parentId);
        List<LightCommand3Control2> results = query.getResultList();
        return results.stream()
                .collect(Collectors.groupingBy(LightCommand3Control2::getAssetCode));
    }

    public  List<LightCommand3Control2> usrSpGetAssetInfoListByParentId( String parentId) {
        return persistenceService.doReturningTransaction(entityManager -> {
            String sql = "SELECT * FROM usr_sp_getAsset_info_list_by_parentId(:parentId)";

            Query query = entityManager.createNativeQuery(sql, LightCommand3Control2.class);
    ...
    public Map<String, List<LightCommand3Control2>> usrSpGetAssetInfoListByParentIdGroupLightAsset(EntityManager em, String parentId) {
        String sql = "SELECT * FROM usr_sp_getAsset_info_list_by_parentId_group_light_asset(:parentId)";
    ...
    public List<LightCommand3Control2> usrSpGetAssetInfoListByParentIdGroupLightAsset(String parentId) {
        return persistenceService.doReturningTransaction(em ->{
            String sql = "SELECT * FROM usr_sp_getAsset_info_list_by_parentId_group_light_asset(:parentId)";
    ...
    public List<Asset<?>> getAssetByAssetCode(EntityManager em, String assetCode) {
        String sql = "select a.* " +
                "from asset_info ai  " +
                "inner join asset a on ai.id =a.id  " +
                "where ai.asset_code =:asset_code ";

        Query query = em.createNativeQuery(sql, Asset.class);
        query.setParameter("asset_code", assetCode);
        return  query.getResultList();
    }

    public List<Asset<?>> getAssetByAssetCode(String assetCode) {
        return persistenceService.doReturningTransaction(em -> getAssetByAssetCode(em, assetCode));
    }

    public List<?> getAssetCodeByAssetId(String id) {
        return persistenceService.doReturningTransaction(em -> {
            String sql = "select ai.asset_code " +
                    "from asset_info ai  " +
                    "where ai.id =:id ";
...
//    public boolean sendDimCommandToSingleLight(EntityManager em, AttributeEvent enrichedEvent) {
//        String sql  = "SELECT a.attributes['nemaId']['value']  FROM asset a WHERE a.id = :idLS AND a.type='LightAsset'";
//        String idLS = enrichedEvent.getRef().getId();
//
//        Query query = em.createNativeQuery(sql);
//        query.setParameter("idLS", idLS);
//        Object result = query.getSingleResult();
//
//        String value = ValueUtil.asJSON(enrichedEvent.getValue().orElse(null)).orElse(ValueUtil.NULL_LITERAL);
//
//        Hdi3ControlSe hdiValue3 = new Hdi3ControlSe(result.toString()+"-"+value);
//        enrichedEvent.setValue(hdiValue3);
...
//    }
        ...
        switch (persistenceEvent.getCause()) {
            case CREATE -> {
                ...
                clientEventService.publishEvent(
                        new AssetEvent(AssetEvent.Cause.CREATE, loadedAsset, null,null)
                );
            ...
            case UPDATE -> {
                ...
                clientEventService.publishEvent(
                        new AssetEvent(AssetEvent.Cause.UPDATE, asset, persistenceEvent.getPropertyNames().toArray(String[]::new), assetInfoDto)
                );
            ...
            case DELETE -> {
                ...
                clientEventService.publishEvent(
                        new AssetEvent(AssetEvent.Cause.DELETE, asset, null,null)
                );
    ...
    protected static String buildOrderByString(AssetQuery query) {
        ...
        if (query.orderBy != null && query.orderBy.property != null) {
            ...
            switch (query.orderBy.property) {
                ...
                case LATEST_UPDATE_DATE -> sb.append(" ABS(EXTRACT(EPOCH FROM NOW()) * 1000 - CAST(a.attributes->'consoleProviders'->>'timestamp' AS BIGINT)) ");
            }
    ...
    public List<FilterDto> getAllRoadAssets(String realm) {
        List<Asset> entitiList = persistenceService.doReturningTransaction(entityManager ->
                entityManager.createQuery("SELECT a FROM Asset a WHERE a.type LIKE 'RoadAsset' AND a.realm = :realm")
                        .setParameter("realm",realm)
                        .getResultList()
        );
        List<FilterDto> filterDtos = new ArrayList<>();

        for (Asset asset : entitiList){
            filterDtos.add(new FilterDto(asset.getId(), asset.getName(),asset.getAssetType(), asset.getParentId()));
        }
        return filterDtos;
    }
    ...
    public List<FilterDto> getAllCabinetAssets(String realm) {
        List<Asset> entitiList = persistenceService.doReturningTransaction(entityManager ->
                entityManager.createQuery("SELECT a FROM Asset a WHERE a.type LIKE 'ElectricalCabinetAsset' AND a.realm = :realm")
                        .setParameter("realm",realm)
                        .getResultList()
        );
    ...
    public List<FilterDto> getAllLightAssets() {
        List<Asset> entitiList = persistenceService.doReturningTransaction(entityManager ->
                entityManager.createQuery("SELECT a FROM Asset a WHERE a.type LIKE 'LightAsset'")
                        .getResultList()
        );
    ...
    public List<FilterDto> getAssetName(List<String> typeList, String realm) {
        List<FilterDto> entitiList = persistenceService.doReturningTransaction(entityManager ->
                entityManager.createNativeQuery("SELECT a.id, a.name, a.type, a.parent_id " +
                                "FROM Asset a inner join Asset_info ai on a.id = ai.id WHERE ai.deleted = false and a.realm = :realm and a.type IN :typeList and (select type from asset where asset.id=subpath(a.path, 0, 1)\\:\\:text  )!='RoadAsset'", FilterDto.class)
                        .setParameter("realm", realm)
                        .setParameter("typeList", typeList)
                        .getResultList()
        );

        return entitiList == null ? new ArrayList<>() : entitiList;
    }

    public List<Asset> getListRoad(String roadId, String realm) {
        return persistenceService.doReturningTransaction(entityManager ->
                entityManager.createQuery(
                                "SELECT a FROM Asset a " +
                                        "WHERE a.type LIKE 'RoadAsset' " +
                                        "AND (:roadId IS NULL OR a.id LIKE :roadId) " +
                                        "AND (a.realm = :realm)", Asset.class)
    ...
    public Asset getRoadByCaninetId(String cabinetId, String realm) {
        List<Object[]> results = (List<Object[]>) persistenceService.doReturningTransaction(entityManager ->
                entityManager.createNativeQuery(
                                "SELECT a.id, a.name FROM Asset a " +
                                        "JOIN (SELECT * FROM Asset ast WHERE ast.id = :cabinetId) cabinet " +
                                        "ON a.id = cabinet.parent_id " +
                                        "AND a.type LIKE 'RoadAsset' " +
                                        "AND a.realm = :realm")
        ...
        if (!results.isEmpty()) {
            ...
            asset.setId(data[0] != null ? String.valueOf(data[0]) : null);
            asset.setName(data[1] != null ? String.valueOf(data[1]) : null);
            return asset;
        }
    ...
    public List<Asset> getListCabinet(String roadId, String realm) {
        return persistenceService.doReturningTransaction(entityManager ->
                entityManager.createQuery(
                                "SELECT a FROM Asset a " +
                                        "WHERE a.type LIKE 'ElectricalCabinetAsset' " +
                                        "AND (:roadId IS NULL OR a.parentId LIKE :roadId) " +
                                        "AND (a.realm = :realm)", Asset.class)
                        .setParameter("roadId", roadId)
                        .setParameter("realm", realm)
                        .getResultList()
    ...
    public List<Asset> getCabinet(String cabinetId) {
        return persistenceService.doReturningTransaction(entityManager ->
                entityManager.createQuery(
                                "select a from Asset a " +
                                        "left join AssetInfoEntity ai on a.id = ai.id " +
                                        "where a.type = 'ElectricalCabinetAsset' " +
                                        "and (:cabinetId is null or a.id = :cabinetId) " +
                                        "and ai.deleted = false ", Asset.class)
                        .setParameter("cabinetId", cabinetId)
                        .getResultList()
        );
    }

    public List<Asset> getAllLightByCabinetId(String cabinetId, String realm) {
        List<Object[]> results = (List<Object[]>) persistenceService.doReturningTransaction(entityManager ->
                entityManager.createNativeQuery(
                                "SELECT a.id, a.name FROM Asset a " +
                                        "LEFT JOIN asset_info ai ON a.id = ai.id " +
                                        "LEFT JOIN asset_cabinet ac ON a.id = ac.asset_id " +
                                        "WHERE ac.cabinet_id LIKE :cabinetId " +
                                        "AND a.type LIKE 'LightAsset' " +
                                        "AND a.realm = :realm")
                        .setParameter("cabinetId", cabinetId)
                        .setParameter("realm", realm)
                        .getResultList()
        ...
        for (Object[] objects : results) {
            Asset asset = new Asset(){};
            asset.setId(objects[0] != null ? String.valueOf(objects[0]) : null);
            asset.setName(objects[1] != null ? String.valueOf(objects[1]) : null);
            assets.add(asset);
        }
    ...
    public boolean updateAttribute(EntityManager em, AttributeEvent event) throws ConstraintViolationException {
        return updateAttributeValue(em, event);
    }

    public LightInfoDTO getInfoLightByLightId (String lightId) {
        LightInfoDTO lightInfoDTO = new LightInfoDTO();
        List<Object[]> results = (List<Object[]>) persistenceService.doReturningTransaction(entityManager ->
                entityManager.createNativeQuery(
                                "WITH brightness_data AS ( " +
                                        "SELECT ad.entity_id, ad.timestamp " +
                                        "FROM asset_datapoint ad " +
                                        "WHERE ad.entity_id = :lightId " +
                                        "AND ad.attribute_name = 'brightness' " +
                                        "AND CAST(ad.value->>'value' AS numeric) > 0 " +
                                        "ORDER BY ad.timestamp DESC " +
                                        "LIMIT 1 " +
                                        ") " +
                                        "SELECT ai.id, ai.firmware_version, bd.timestamp AS lighting_time " +
                                        "FROM asset_info ai " +
                                        "LEFT JOIN brightness_data bd ON ai.id = bd.entity_id " +
                                        "WHERE ai.id = :lightId")
        ...
        if (results != null && !results.isEmpty()) {
            Object[] data = results.get(0);
            lightInfoDTO.setLightId(data[0] != null ? (String) data[0] : "");
            lightInfoDTO.setFirmwareVersion(data[1] != null ? (String) data[1] : "");
            lightInfoDTO.setTimeLight(data[2] != null ? (Date) data[2] : new Date(0));
...
//        return null;
    ...
    public CabinetAssetDTO create(CabinetAssetDTO asset) {
        try {
            return persistenceService.doReturningTransaction(em -> {
                ElectricalCabinetAsset cabinetAsset = asset.getCabinetAsset();
                ...
                if (validationUtils.isValid(cabinetAsset.getId())) {
    ...
    public CabinetAssetDTO update(CabinetAssetDTO cabinetAsset) {
        persistenceService.doReturningTransaction(em -> {
            ObjectMapper objectMapper = new ObjectMapper();
            ElectricalCabinetAsset asset = cabinetAsset.getCabinetAsset();
            Asset_Info assetInfo = cabinetAsset.getAssetInfo();
            RouteInfo routeInfo = cabinetAsset.getRouteInfo();

            String parentId = null;
            if (validationUtils.isValid(routeInfo) && validationUtils.isValid(routeInfo.getId())) {
                parentId = routeInfo.getId();
            }
            String[] pathArray = asset.getPath();
            String pathAsString = String.join(".", pathArray);
            try {
                em.createNativeQuery(
                                "UPDATE asset " +
                                        "SET attributes = CAST(? AS jsonb), name = ?, parent_id = ?, path = CAST(? AS ltree), access_public_read = ?, version =? " +
                                        "WHERE id = ?")
                        .setParameter(1, objectMapper.writeValueAsString(asset.getAttributes()))
                        .setParameter(2, asset.getAssetName())
                        .setParameter(3, parentId)
                        .setParameter(4, pathAsString)
                        .setParameter(5, asset.isAccessPublicRead())
                        .setParameter(6, asset.getVersion())
                        .setParameter(7, asset.getId())
                        .executeUpdate();
            ...
            em.createNativeQuery("""
                                ...
                                WHERE id = ?
                            """)
                    .setParameter(1, assetInfo.getAssetCode())
                    .setParameter(2, LocalDateTime.now()) // update_date
                    .setParameter(3, assetInfo.getUpdateBy())
                    .setParameter(4, assetInfo.getStatus())
                    .setParameter(5, assetInfo.getAssetModel())
                    .setParameter(6, assetInfo.getId()) // WHERE id = ?
                    .executeUpdate();

            if (validationUtils.isValid(routeInfo) && validationUtils.isValid(routeInfo.getId())) {

                deleteRouterAsset(em, asset.getId());
                ...
                em.createNativeQuery("""
                            ...
                            """)
                        .setParameter(1, routeInfo.getId())
                        ...
                        .setParameter(10, LocalDate.now())
                        .setParameter(11, assetInfo.getCreateBy())
                        .executeUpdate();

                updateParent(em, routeInfo.getId(), asset.getId());
            } else {
                updateParent(em, null, asset.getId());
    ...
    private void deleteRouterAsset(EntityManager em, String id) {
        em.createNativeQuery("DELETE FROM route_assets " +
                        "WHERE asset_id = ?")
                .setParameter(1, id)
                .executeUpdate();
    }
