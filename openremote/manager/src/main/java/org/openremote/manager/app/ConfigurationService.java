...
public class ConfigurationService extends RouteBuilder implements ContainerService {
    ...
    public String saveImageFileExtend(String path, FileInfo fileInfo) {
        LOG.log(Level.INFO, "Saving image in manger_config.json..");
        String pathName = "/upload/" + getFirmwarePath(fileInfo.isFirmware()) + path;
        ...
        return file.exists() ? file.getPath().replace("\\", "/") : null;
    ...
    private String getFirmwarePath(boolean isFirmware) {
        String basePath = persistenceService.doReturningTransaction(em ->
                em.createNativeQuery(
                                "SELECT parm_value FROM sys_config_parms WHERE sys_param_code = 'firmware_path'")
        ...
        return isFirmware? basePath + "/" + datePath + "/" + System.currentTimeMillis(): datePath + "/" + System.currentTimeMillis();
    }
