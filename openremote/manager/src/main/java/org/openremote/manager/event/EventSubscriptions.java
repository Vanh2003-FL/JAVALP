...
//           String viet1= subscription.getEventType();
//           String viet2= event.getEventType();
            return subscription.getEventType().equals(event.getEventType());
        }
        public boolean hdiMatches(SharedEvent event) {
//            String viet3= subscription.getHdiEventType();
//            String viet4= event.getEventType();
            return subscription.getHdiEventType() == null || !subscription.getHdiEventType().equals(event.getEventType());
        }
    }
        ...
        for (Map.Entry<String, SessionSubscriptions> entry : sessionSubscriptionIdMap.entrySet()) {
            ...
            for (SessionSubscription<?> sessionSubscription : subscriptions) {
                ...
                if (!sessionSubscription.matches(event)) {
                    if (sessionSubscription.hdiMatches(event))
                        continue;
                }
