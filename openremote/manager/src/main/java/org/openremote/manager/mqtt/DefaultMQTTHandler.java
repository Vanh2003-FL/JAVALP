...
public class DefaultMQTTHandler extends MQTTHandler {
    ...
    public boolean hdiTopicSub(Topic topic) {
        return isControlTopic(topic)||isStatusTopic(topic)||isPublishTopic(topic);
    }
    public boolean hdiTopicPub(Topic topic) {
        return isDataTopic(topic)||isStatusTopic(topic);
    }
    ...
    public boolean canSubscribe(RemotingConnection connection, KeycloakSecurityContext securityContext, Topic topic) {
        ...
        boolean isControlTopic = isControlTopic(topic);
        boolean hdiTopicSub=hdiTopicSub(topic);

        if (!isAssetTopic && !isAttributeTopic && !hdiTopicSub) {
        ...
        if (filter == null && !hdiTopicSub && !hdiTopicPub(topic)) {
    ...
    public boolean canPublish(RemotingConnection connection, KeycloakSecurityContext securityContext, Topic topic) {
        boolean isAttributeValueWriteTopic=isAttributeValueWriteTopic(topic);
        boolean hdiTopicPub=hdiTopicPub(topic);
        boolean isDataTopic=isDataTopic(topic);
        if (!isKeycloak) {
        ...
        if (!isAttributeValueWriteTopic && !hdiTopicPub) {
            LOG.finer("Không thể pub lên topic này");
            return false;
        }

        if (isAttributeValueWriteTopic) {
        ...
        if(isDataTopic){
            if (topic.getTokens().size() != 3) {
                LOG.finer("Topic không đúng cấu trúc");
                return false;
            }
        }
        ...
        if(!hdiTopicPub){
            ...
            if (!clientEventService.authorizeEventWrite(topicRealm(topic), authContext, hdiBuildAttributeEvent(topic.getTokens(), null))) {
                LOG.fine("Publish was not authorised for this user and topic: topic=" + topic + ", subject=" + authContext);
                return false;
            }
        }
    ...
    public void onSubscribe(RemotingConnection connection, Topic topic) {
        ...
        if(hdiTopicSub){
            subscription.setHdiEventType(HdiEven.class);
        }
    ...
    public Set<String> getPublishListenerTopics() {
        return Set.of(
                ...
                TOKEN_SINGLE_LEVEL_WILDCARD + "/" + TOKEN_SINGLE_LEVEL_WILDCARD + "/" + DATA_TOPIC,
                TOKEN_SINGLE_LEVEL_WILDCARD + "/" + TOKEN_SINGLE_LEVEL_WILDCARD + "/" + STATUS_TOPIC
        );
    }
//v
    @Override
    public void onPublish(RemotingConnection connection, Topic topic, ByteBuf body) {
        ...
        if (isDataTopic(topic)) {
            attributeEvent = hdiBuildAttributeEvent(topicTokens, value);
            Map<String, Object> headers = prepareHeaders(topicRealm(topic), connection);
        ...
        if(isStatusTopic(topic)){
            HdiEven hdiEven= new HdiEven(payloadContent);
            hdiEven.setAssetCode(topicTokenIndexToString(topic,1));
            Map<String, Object> headers = prepareHeaders(topicRealm(topic), connection);
            messageBrokerService.getFluentProducerTemplate()
                    .withHeaders(headers)
                    .withBody(hdiEven)
        ...
        attributeEvent = buildAttributeEvent(topicTokens, value);
        Map<String, Object> headers = prepareHeaders(topicRealm(topic), connection);
        LOG.finer("Publishing to client inbound queue: " + attributeEvent);
        messageBrokerService.getFluentProducerTemplate()
                .withHeaders(headers)
    ...
    protected  AttributeEvent hdiBuildAttributeEvent(List<String> topicTokens, Object value) {
        String assetCode = topicTokens.get(1);
        Asset<?>asset= assetStorageService.getAssetByAssetCode(assetCode).get(0);
        String attributeName = AssetProcessingService.BRIGHTNESS;
        String assetId = asset.getId();
        return new AttributeEvent(assetId, attributeName, value,assetCode).setSource(DefaultMQTTHandler.class.getSimpleName());
    }
    ...
    protected Consumer<SharedEvent> getSubscriptionEventConsumer(RemotingConnection connection, Topic topic) {
        ...
        boolean isAttributeTopic = isAttributeTopic(topic);
        boolean isControlTopic = isControlTopic(topic);
        boolean isPublishTopic = isPublishTopic(topic);
        ...
        return ev -> {
            if (ev instanceof HdiEven hdiEven) {
                if (isControlTopic) {
                    if (hdiEven.getValue() instanceof Hdi3SceneClear hdi3SceneClear && clientId.equals(hdiEven.getAssetCode())) {
                        mqttBrokerService.publishMessage(topicExpander.apply(ev), hdi3SceneClear, mqttQoS);
                    }
                    if (hdiEven.getValue() instanceof Hdi3TimerSe hdi3TimerSe && clientId.equals(hdiEven.getAssetCode())) {
                        mqttBrokerService.publishMessage(topicExpander.apply(ev), hdi3TimerSe, mqttQoS);
                    }
                    if (hdiEven.getValue() instanceof Hdi3Power hdi3Power && clientId.equals(hdiEven.getAssetCode())) {
                        mqttBrokerService.publishMessage(topicExpander.apply(ev), hdi3Power, mqttQoS);
                    }
                    if (hdiEven.getValue() instanceof Hdi3SceneSe hdi3SceneSe && clientId.equals(hdiEven.getAssetCode())) {
                        mqttBrokerService.publishMessage(topicExpander.apply(ev), hdi3SceneSe, mqttQoS);
                    }

                    if (hdiEven.getValue() instanceof Hdi250DataDe hdi250DataDe && clientId.equals(hdiEven.getAssetCode())) {
                        mqttBrokerService.publishMessage(topicExpander.apply(ev), hdi250DataDe, mqttQoS);
                    }
                    if (hdiEven.getValue() instanceof Hdi4State hdi4State && clientId.equals(hdiEven.getAssetCode())) {
                        mqttBrokerService.publishMessage(topicExpander.apply(ev), hdi4State, mqttQoS);
                    }

                    if (hdiEven.getValue() instanceof Hdi3ControlSe2 hdi3ControlSe2 && clientId.equals(hdiEven.getAssetCode())) {
                        mqttBrokerService.publishMessage(topicExpander.apply(ev), hdi3ControlSe2, mqttQoS);
                    }
                    if (hdiEven.getValue() instanceof Hdi5Ota hdi5Ota && clientId.equals(hdiEven.getAssetCode())) {
                        mqttBrokerService.publishMessage(topicExpander.apply(ev), hdi5Ota, mqttQoS);
                    }
                }
                if(isPublishTopic){
                    mqttBrokerService.publishMessage(topicExpander.apply(ev), hdiEven.getValue(), mqttQoS);
                }
            }
            if (ev instanceof AttributeEvent attributeEvent) {
                boolean isHdi = attributeEvent.getValue().orElse(null) instanceof HdiCmdData;
                if (isAttributeTopic) {
                    if (!isHdi) {
                ...
                if (isControlTopic) {
                    if (isHdi && clientId.equals(attributeEvent.getAssetCode())) {
                        mqttBrokerService.publishMessage(topicExpander.apply(ev), attributeEvent.getValue().orElse(null), mqttQoS);
                    }
                }
                if(isPublishTopic){
    ...
    protected static boolean isControlTopic(Topic topic) {
        return CONTROL_TOPIC.equalsIgnoreCase(topicTokenIndexToString(topic, 2));
    }
    private boolean isDataTopic(Topic topic) {
        return DATA_TOPIC.equalsIgnoreCase(topicTokenIndexToString(topic, 2));
    }

    private boolean isStatusTopic(Topic topic) {
        return STATUS_TOPIC.equalsIgnoreCase(topicTokenIndexToString(topic, 2));
    }
    private boolean isPublishTopic(Topic topic) {
        return PUBLISH_TOPIC.equalsIgnoreCase(topicTokenIndexToString(topic, 2));
    }
