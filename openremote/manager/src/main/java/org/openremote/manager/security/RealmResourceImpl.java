...
public class RealmResourceImpl extends ManagerWebResource implements RealmResource {
    ...
    public RealmResourceImpl(TimerService timerService, ManagerIdentityService identityService, Container container,PersistenceService persistenceService) {
    ...
    @Override
    public List<RealmDTO> getAllNotToken(RequestParams requestParams) {

        return persistenceService.doReturningTransaction(entityManager -> {
            String sql = "select r.id, r.name ,ra.value " +
                    "from public.realm r " +
                    "inner join PUBLIC.REALM_ATTRIBUTE ra on ra.REALM_ID = r.id and ra.name = 'displayName'";
            Query query = entityManager.createNativeQuery(sql,RealmDTO.class);
    ...
    public void create(RequestParams requestParams, Realm realm) {
        ...
        try {
            if(realm.getLoginTheme().isEmpty()||realm.getLoginTheme()==null){
                realm.setLoginTheme("custom2");
            }
