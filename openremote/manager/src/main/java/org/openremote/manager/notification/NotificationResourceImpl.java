...
public class NotificationResourceImpl extends WebResource implements NotificationResource {
    ...
    @Override
    public List<NotificationDTO> getAll(SearchFilterDTO<SentNotification> filterDTO) {
        return notificationService.getAll(filterDTO);
    }

    @Override
    public Long countData(SearchFilterDTO<SentNotification> filterDTO) {
        return notificationService.count(filterDTO);
    }

    @Override
    public List<SentNotification> getSentNotifications(String userId) {
        return notificationService.getSentNotifications(userId);
    }

    @Override
    public void markAsRead(RequestParams requestParams, Long notificationId) {
        if (notificationId == null) {
            throw new WebApplicationException("Missing notification ID", BAD_REQUEST);
        }
