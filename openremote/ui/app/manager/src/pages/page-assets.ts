...
import {AssetEventCause, AssetInfoDto, WellknownAssets} from "@openremote/model";
...
export class PageAssets extends Page<AssetsStateKeyed>  {
    ...
    @state() // selected asset ids
    protected _assetChild?: string[];
    ...
    protected _onAssetSelectionChanged(event: OrAssetTreeSelectionEvent) {
        ...
        const getAllIds = (nodes) => {
            return nodes.flatMap(node => [
                ...(node.asset?.id ? [node.asset.id] : []),
                ...(node.children ? getAllIds(node.children) : [])
            ]);
        };

        const child1 = getAllIds(event.detail.newNodes);
        child1.shift()
        // const child1 = event.detail.newNodes
        //     .flatMap((node) => node.children || []) // Đảm bảo không có giá trị undefined
        //     .map((child) => child.asset!.id);
        this._assetChild = child1
        console.log('test child',child1)
        this._viewer.ids = this._assetIds;
        this._updateRoute(true);
    }
    ...
    protected async _onAssetAdd(ev: OrAssetTreeAddEvent) {
        if (this._editMode) {
            ...
            // Auto save and load
            const result = await saveAsset(ev.detail.asset,this._assetChild);
    ...
    protected async _onAssetParentChange(newParentId: any) {
      ...
      console.log('abcd',this._assetChild)
        try {
            if (parentId) {
                if ( !assetsIds.includes(parentId) ) {
                    await manager.rest.api.AssetResource.updateParent(parentId, { assetIds : assetsIds,childIds:this._assetChild });
                } else {
