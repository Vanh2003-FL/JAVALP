...
export class PageUsers extends Page<AppStateKeyed> {
    ...
    private async _createUpdateUser(user: UserModel, action: 'update' | 'create'): Promise<boolean> {
        ...
        try {
            ...
            if (isAxiosError(e)) {
                console.error((isUpdate ? "save user failed" : "this username already exists on another realm") + ": response = " + e.response.statusText);
                if (e.response.status === 400) {
                    showSnackbar(undefined, (isUpdate ? "saveUserFailed" : "this username already exists on another realm"), "dismiss");
