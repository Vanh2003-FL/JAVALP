...
export class PageExport extends Page<AppStateKeyed> {
            ...
            Promise.all(assetInfoPromises).then(assetInfos => {
                ...
                if (allDatapoints.length > 0) {
                    this.tableRows = allDatapoints.map(dataInfo => {
                        const relevantAssetInfo = allAssets.find(asset => asset.asset?.id === dataInfo.assetId);
                        return {
                            assetName: relevantAssetInfo.asset?.name,
                            assetId: relevantAssetInfo.asset?.id,
    ...
    protected async loadConfig() {
        ...
        const assetsIds = response.data.map(e => e.asset?.id) || [];
