...
@customElement('report-tab-voltage')
export class ReportTabVoltage extends LitElement {
    @state() dataRoad = []
    @state() dataCabinet = []
    @state() idRoad = ""
    @state() idCabinet = ""
    ...
    exportToPDF() {
        ...
        const docDefinition = {
            content: [
                { text: "BÁO CÁO Điện áp tiêu thụỤ THEO KỲ", style: "title" },
    ...
    async exportToExcel() {
        ...
        titleCell.value = `Báo cáo điệp áp tiêu thụ từ ${this.objectDate.fromDate} đến ${this.objectDate.toDate}`;
        ...
        const titleRow = [
            ...
            "Điện áp tiêu thụ", // gộp nhiều giờ thành 1
        ...
        saveAs(dataBlob, "BaoCaoDienApTieuThu.xlsx");
    }
        ...
        const groupedData = this.dataTable.map((cabinet) => {
            ...
            (cabinet.powerVoltageReport ?? []).forEach((light) => {
                ...
                if (this.dates.includes(dayString)) {
                    existing.days[dayString] = `${light.voltage ?? 0}`;
                }
                existing.total += light.voltage ?? 0;
            });
    ...
    render() {
        return html`
            ...
            <div style="padding:10px 0px; border-bottom:1px solid #e3e6ea; margin:20px; background:white; border-radius:10px">
                <div>
            ...
            <div style="background:white; padding:15px 20px; margin:20px; border-radius:10px;">
                <h2 style="margin-top:0px;">
                    ${this.selectedKBC === "D"
                            ? html`<or-translate value="detailed_voltage_consumption_report"></or-translate> ${this.objectDate.fromDate}`
                            : html`<or-translate value="summary_voltage_consumption_report"></or-translate> ${
                                    this.selectedKBC === "M" ? `tháng ${this.selectedMonth}/${this.selectedYear}` :
                                            this.selectedKBC === "Y" ? `năm ${this.selectedYear}` :
                                                    this.selectedKBC === "Q" ? `quý ${this.selectedQuarter} năm ${this.selectedYear}` :
                                                            this.selectedKBC === "C" ? `từ ${this.objectDate.fromDate} đến ${this.objectDate.toDate}` :
                                                                    ''
                            }`
                    }
                </h2>
                    ...
                    <table>
                        ...
                        <tr>
                            ...
                            <th colspan="${this.dates.length}" style="text-align:center;">
                                <or-translate value="power_voltage"></or-translate>
                            </th>
