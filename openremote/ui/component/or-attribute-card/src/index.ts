...
import {
    ...
    ValueDatapoint,
    AssetModelUtil,
    AssetDatapointLTTBQuery, AssetInfoDto
...
const style = css`
    ...
    .chart-wrapper canvas {
        /* Don't hide tooltip - only block labels rendered on canvas */
    }
    .controls-wrapper {
        height: 100%;
        display: flex;
...
export class OrAttributeCard extends LitElement {
    ...
    private _originalFillText?: (text: string, x: number, y: number, maxWidth?: number) => void;
    private _originalStrokeText?: (text: string, x: number, y: number, maxWidth?: number) => void;
    ...
    protected _blockDataPointLabels(ctx: CanvasRenderingContext2D) {
        ...
        // Block ONLY labels with "x:" or "y:" pattern, but allow simple numbers and tooltips
        ctx.fillText = (text: any, x: number, y: number, maxWidth?: number) => {
            const textStr = String(text || '').trim();
            
            // NEVER block tooltip text - tooltips are rendered outside canvas, but just in case
            // Block patterns with "x:" or "y:" - these are the cluttered labels
            const shouldBlock = 
                (textStr.includes('x:') || textStr.includes('y:')) && 
                (/^\d+,\s*y:\s*\d+/.test(textStr) || 
                 /x:\s*\d+/.test(textStr) ||
                 /^\d+,\s*y:\s*\d+\.\d+/.test(textStr) ||
                 /x:\s*\d+,\s*y:\s*\d+/.test(textStr) ||
                 // Block long comma-separated number strings with x: or y:
                 /^\d+,\d+,\d+/.test(textStr) ||
                 // Block very long numbers with x: or y: (timestamps)
                 /^\d{13,}/.test(textStr) ||
                 /\d+,\s*y:\s*\d+/.test(textStr) ||
                 /x:\s*\d+/.test(textStr));
            
            // Allow simple numbers, dates, and other text (tooltips, axis labels, etc.)
            if (shouldBlock) {
                return;
            }
            
            // Allow all other text
            if (this._originalFillText) {
        ...
        ctx.strokeText = (text: any, x: number, y: number, maxWidth?: number) => {
            ...
            // Block only patterns with "x:" or "y:"
            const shouldBlock = 
                textStr.includes('x:') || 
                textStr.includes('y:') || 
                /^\d+,\s*y:\s*\d+/.test(textStr) || 
    ...
    updated(changedProperties: PropertyValues) {
        ...
        if (!this._chart) {
            ...
            // Block data point labels BEFORE creating chart
                            ...
                            callbacks: {
                                title: (context: any) => {
                                    if (context && context.length > 0) {
                                        ...
                                        if (point.parsed && point.parsed.x) {
                                            return moment(point.parsed.x).format('DD/MM/YYYY HH:mm:ss');
                                        }
                                    }
                                    return "";
                                },
                                label: (context: any) => {
                                    if (context.parsed && context.parsed.y !== null && context.parsed.y !== undefined) {
                                        ...
                                        const decimals = this.mainValueDecimals || 0;
                                        let unit = "";
                                        if (this.assets && this.assetAttributes && this.assets.length > 0 && this.assetAttributes.length > 0) {
                                            ...
                                            const attributeDescriptor = AssetModelUtil.getAttributeDescriptor(attr.name!, this.assets[0].asset?.type!);
                                            unit = Util.resolveUnits(Util.getAttributeUnits(attr, attributeDescriptor, this.assets[0].asset?.type)) || "";
                                        }
            ...
            // Re-apply blocking after chart creation
            this._blockDataPointLabels(ctx);
        } else {
            if (changedProperties.has("data") && this._chart && this._chartElem) {
                this._chart.data.datasets![0].data = this.data.filter(value => value.y != null) as ScatterDataPoint[];
                this._chart.update('none');
                
                // Re-apply blocking after update
            ...
            <div class="panel" id="attribute-card">
                <div class="panel-content-wrapper">
                    <div class="panel-title">
                        ${this.shouldShowTitle() ? html`<span class="panel-title-text">${this.assets[0].asset?.name + " - " + i18next.t(this.assetAttributes[0][1].name!)}</span>` : undefined}
                    ...
                    <div class="panel-content">
                        <div class="mainvalue-wrapper">
                            <span class="main-number-icon">${this.assets && this.assets.length === 1 ? getAssetDescriptorIconTemplate(AssetModelUtil.getAssetDescriptor(this.assets[0].asset?.type!)) : ""}</span>
    ...
    private _setAttribute(attributeName: string) {
        if (this.asset && attributeName) {
            const attr = this.asset.asset?.attributes ? this.asset.asset?.attributes[attributeName] : undefined;
    ...
    protected async loadData() {
        ...
        const assetId = this.assets[0].asset?.id!;
    ...
    protected getFormattedValue(value: number | undefined): {value: number, unit: string} | undefined {
        ...
        const attributeDescriptor = AssetModelUtil.getAttributeDescriptor(attr.name!, this.assets[0].asset?.type!);
        const units = Util.resolveUnits(Util.getAttributeUnits(attr, attributeDescriptor, this.assets[0].asset?.type));
