...
import {Asset, Attribute, AssetDatapointLTTBQuery} from "@openremote/model";
...
export class OrDataViewer extends translate(i18next)(LitElement) {
    ...
    public getPanelContent(panelName: string,  panelConfig: PanelConfig): TemplateResult | undefined {
        ...
        if (panelConfig && panelConfig.type === "chart") {
            const defaultDate = moment().subtract(24, 'hours').toDate();
            const now = new Date();
            const defaultDatapointQuery: AssetDatapointLTTBQuery = {
                type: "lttb",
                fromTimestamp: defaultDate.getTime(),
                toTimestamp: now.getTime(),
                amountOfPoints: 100
            };
            content = html`<or-chart id="chart" panelName="${panelName}" .config="${this.config.chartConfig}" .realm="${this.realm}" .datapointQuery="${defaultDatapointQuery}"></or-chart>`;
        }
