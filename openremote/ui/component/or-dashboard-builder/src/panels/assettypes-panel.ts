...
import {Asset, AssetDescriptor, AssetInfoDto, AssetModelUtil, AssetQuery, AssetTypeInfo} from "@openremote/model";
...
export class AssettypesPanel extends LitElement {
    ...
    protected _openAssetSelector(assetType: string, assetIds?: string[], multi = false) {
        ...
        const dialog = showDialog(new OrMwcDialog()
            ...
            .setContent(html`
                <div style="width: 500px; min-width: 400px;">
                    <or-asset-tree ${ref(assetTreeRef)} .dataProvider="${this.assetTreeDataProvider}" .expandAllNodes="${true}"
            ...
            .setActions([
                ...
                },
                {
                    ...
                    action: () => {
                        ...
                        console.log('OK clicked, tree:', tree, 'selectedIds:', tree?.selectedIds);
                        if(tree?.selectedIds) {
                            if(multi) {
                            ...
                            // willUpdate will automatically dispatch AssetIdsSelectEvent when assetIds changes
                        }
                        return true;
                    }
                }
        ...
        // Set expandAllNodes after dialog is shown
        console.log('_openAssetSelector - assetTreeRef:', assetTreeRef, 'assetTreeRef.value:', assetTreeRef.value);
        setTimeout(() => {
            console.log('setTimeout callback - assetTreeRef.value:', assetTreeRef.value);
            const tree = assetTreeRef.value;
            if (tree) {
                console.log('Setting expandAllNodes to true, tree:', tree, 'current expandAllNodes:', tree.expandAllNodes);
                tree.expandAllNodes = true;
                console.log('After setting, expandAllNodes:', tree.expandAllNodes);
                tree.requestUpdate();
            } else {
                console.log('assetTreeRef.value is null/undefined, cannot set expandAllNodes');
            }
        }, 100);
        
        // Also try after a longer delay in case the first one doesn't work
        setTimeout(() => {
            const tree = assetTreeRef.value;
            if (tree) {
                console.log('Second setTimeout - Setting expandAllNodes to true, tree:', tree, 'current expandAllNodes:', tree.expandAllNodes);
                tree.expandAllNodes = true;
                console.log('After second setting, expandAllNodes:', tree.expandAllNodes);
                tree.requestUpdate();
            }
        }, 500);
    }

    protected assetTreeDataProvider = async (): Promise<AssetInfoDto[]> => {
        ...
        // If assetType is not set, return all assets (like production)
        if (!this.assetType) {
            return assets;
        }
        ...
        const pathsOfAssetType = assets.filter(a => a.asset?.type === this.assetType).map(a => a.asset?.path!);
        const filteredAssetIds = [...new Set([].concat(...pathsOfAssetType as any[]))] as string[];
        return assets.filter(a => filteredAssetIds.includes(a.asset?.id!));
    }
