...
import {Asset, AssetInfo, AssetInfoDto, AssetModelUtil, Attribute, AttributeRef} from "@openremote/model";
...
export class AttributeActionEvent extends CustomEvent<{ asset: AssetInfoDto, attributeRef: AttributeRef, action: AttributeAction }> {

    public static readonly NAME = "attribute-action"

    constructor(asset: AssetInfoDto, attributeRef: AttributeRef, action: AttributeAction) {
...
export class AttributesSelectEvent extends CustomEvent<{ assets: AssetInfoDto[], attributeRefs: AttributeRef[] }> {

    public static readonly NAME = "attribute-select";

    constructor(assets: AssetInfoDto[], attributeRefs: AttributeRef[]) {
...
export class AttributesPanel extends LitElement {
    ...
    @property()
    protected attributeLabelCallback?: (asset: AssetInfoDto, attribute: Attribute<any> | undefined, attributeLabel: string) => TemplateResult;

    @property()
    protected attributeActionCallback?: (attribute: AttributeRef) => AttributeAction[]

    @state()
    protected loadedAssets: AssetInfoDto[] = [];
    ...
    protected getLoadedAsset(attrRef: AttributeRef): AssetInfoDto | undefined {
        return this.loadedAssets?.find((asset) => asset.asset?.id === attrRef.id)
    }
    ...
    protected async loadAssets(): Promise<AssetInfoDto[]> {
    ...
    async fetchAssets(attributeRefs: AttributeRef[] = []): Promise<AssetInfoDto[]> {
        let assets: AssetInfoDto[] = [];
    ...
    protected onAttributeActionClick(asset: AssetInfoDto, attributeRef: AttributeRef, action: AttributeAction) {
                    ...
                    <div id="attribute-list">
                        ${guard([this.attributeRefs, this.loadedAssets, this.attributeActionCallback, this.attributeLabelCallback], () => html`
                            ${map(this.attributeRefs, (attributeRef: AttributeRef) => {
                                ...
                                if (asset) {
                                    const attribute = asset.asset?.attributes![attributeRef.name!];
                                    const descriptors = AssetModelUtil.getAttributeAndValueDescriptors(asset.asset?.type, attributeRef.name, attribute);
                                    const label = Util.getAttributeLabel(attribute, descriptors[0], asset.asset?.type, true);
                                    return html`
                                        <div class="attribute-list-item">
                                            <div class="attribute-list-item-icon">
                                                <span>${getAssetDescriptorIconTemplate(AssetModelUtil.getAssetDescriptor(asset.asset?.type))}</span>
                                            </div>
