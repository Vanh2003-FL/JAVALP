...
export class OrDashboardBrowser extends LitElement {
    ...
    protected renderGrid() {
        ...
        const sidebarItems: any[] = Array.from(widgetTypes)
            ...
            .map((typeArr, index) => {
                return {x: coords[index][0], y: coords[index][1], w: 2, h: 2, widgetTypeId: typeArr[0], locked: true, content: `<div class="sidebarItem"><or-icon icon="${typeArr[1].displayIcon}"></or-icon><span class="itemText"><or-translate value="${typeArr[1].displayName}"></or-translate></span>`}
        });
