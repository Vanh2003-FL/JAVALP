...
export class AttributeInputWidget extends OrAssetWidget {
    ...
    protected render(): TemplateResult {
        ...
        const attribute = (config.attributeRefs.length > 0 && this.loadedAssets[0]?.asset?.attributes) ? this.loadedAssets[0].asset?.attributes[config.attributeRefs[0].name!] : undefined;
                ...
                return html`
                    <div id="widget-wrapper">
                        <or-attribute-input class="attr-input" fullWidth
                                            .assetType="${this.loadedAssets[0]?.asset?.type}"
                                            .attribute="${attribute}"
                                            .assetId="${this.loadedAssets[0]?.asset?.id}"
