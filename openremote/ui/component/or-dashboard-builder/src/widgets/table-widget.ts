...
import {Asset, AssetInfoDto, AssetModelUtil} from "@openremote/model";
...
export class TableWidget extends OrAssetWidget {
    ...
    protected getColumns(attributeNames: string[]): TableColumn[] {
        const referenceAsset: AssetInfoDto | undefined = this.loadedAssets[0];
        const attrColumns = attributeNames.map(attrName => {
            let text = attrName;
            let numeric = false;
            if(this.widgetConfig.assetType && referenceAsset && referenceAsset.asset?.attributes && referenceAsset.asset?.attributes[attrName]) {
                const attributeDescriptor = AssetModelUtil.getAttributeDescriptor(attrName, this.widgetConfig.assetType);
                text = Util.getAttributeLabel(referenceAsset.asset?.attributes[attrName], attributeDescriptor, this.widgetConfig.assetType, true);
    ...
    protected getRows(attributeNames: string[]): TableRow[] {
        return this.loadedAssets.map(asset => {
            const attrEntries = attributeNames.map(attrName => {
                if(asset.asset?.attributes && asset.asset?.attributes[attrName]) {
                    const attributeDescriptor = AssetModelUtil.getAttributeDescriptor(attrName, asset.asset?.type!);
                    const strValue = Util.getAttributeValueAsString(asset.asset?.attributes[attrName], attributeDescriptor, asset.asset?.type, false);
            ...
            const content: any[] = Array.of(asset.asset?.name!, ...attrEntries);
            return {
                content: content
            };
        })
