    ...
    private draftCoordinateEntries(config: ImageWidgetConfig): TemplateResult[] {
        ...
        if (config.markers.length > 0) {
            return config.attributeRefs.map((attributeRef) => {
                ...
                const asset = this.loadedAssets?.find(a => a.asset?.id === attributeRef.id);
                ...
                if(asset) {
                    ...
                    label = Util.getAttributeLabel(attribute, descriptors[0], asset.asset?.type, false);
                }
                return html`
                    <div id="marker-container">
                        <div style="flex: 1; display: flex; flex-direction: column;">
                            <span>${this.loadedAssets?.find(a => a.asset?.id === attributeRef.id)?.asset?.name}</span>
                            ${when(label, () => html`
