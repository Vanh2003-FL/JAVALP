...
export class MapSettings extends AssetWidgetSettings {
    ...
    protected async onAttributeNameSelect(ev: AttributeNamesSelectEvent) {
        ...
        await manager.rest.api.AssetResource.queryAssets({
            ...
            this.widgetConfig.assetIds = response.data.map((a) => a.asset?.id!);
            this.widgetConfig.valueType = (response.data.length > 0) ? response.data[0].asset?.attributes![attrName].type : "text"; // sometimes no asset exists of that assetType, so using 'text' as fallback.
        }).catch((reason) => {
