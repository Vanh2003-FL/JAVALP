...
import {Asset, AssetInfoDto, AssetModelUtil, Attribute, AttributeRef} from "@openremote/model";
...
export class OrGauge extends LitElement {
    ...
    willUpdate(changedProps: Map<string, any>) {
        if(changedProps.has('assetAttribute')) {
            ...
            const attributeDescriptor = AssetModelUtil.getAttributeDescriptor(attr.name!, this.asset!.asset?.type!);
            this.unit = Util.resolveUnits(Util.getAttributeUnits(attr, attributeDescriptor, this.asset!.asset?.type));
                    ...
                    <div id="details-container">
                        ${showLabel ? html`
                            <div class="mainvalue-wrapper">
                                <span class="main-number-icon">${this.asset ? getAssetDescriptorIconTemplate(AssetModelUtil.getAssetDescriptor(this.asset.asset?.type)) : ""}</span>
