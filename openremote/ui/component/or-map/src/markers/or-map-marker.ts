...
import {DefaultBoxShadow, manager, subscribe, Util} from "@openremote/core";
import {AssetEvent, AssetInfoDto, AttributeEvent, SharedEvent} from "@openremote/model";
...
export class OrMapMarker extends subscribe(manager)(LitElement) {
    ...
    public _onEvent(event: SharedEvent) {
        ...
        if (event.eventType === "asset") {
            ...
            if (assetEvent.assetInfoDto?.asset?.id === this.asset?.asset?.id) {
                this.asset = assetEvent.assetInfoDto;
                this.requestUpdate();
            }
        }
        if (event.eventType === "attribute" && this.asset) {
    ...
    protected firstUpdated() {
        console.log('this.displayValue', this.asset);
    }

    protected static _defaultTemplate = (icon: string | undefined, options?: TemplateOptions, asset?: any) => `
        ...
        <or-icon class="${asset?.asset?.attributes?.assetStatus?.value === 'D' || asset?.asset?.attributes?.assetStatus?.value == 'D'  ? 'marker-icon blink-red-yellow' : 'marker-icon'}"  icon="${icon || ""}" style="${asset?.asset?.attributes?.assetStatus?.value === 'A' && asset?.asset?.type ==="LightAsset" ? `color:yellow`:`color:black`}"></or-icon>
    `;
    ...
    protected shouldUpdate(_changedProperties: PropertyValues): boolean {
        if (_changedProperties.has("icon") || _changedProperties.has("displayValue") || _changedProperties.has("direction") || _changedProperties.has("asset")) {
    ...
    protected createDefaultMarkerContent(): HTMLElement {
        ...
        console.log('this.displayValue', this.asset);
        div.innerHTML = OrMapMarker._defaultTemplate(this.icon, {displayValue: this.displayValue, direction: this.direction}, this.asset);
        return div;
    }
