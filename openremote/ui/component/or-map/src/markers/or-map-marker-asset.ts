...
export class OrMapMarkerAsset extends subscribe(manager)(OrMapMarker) {
    ...
    protected set type(type: string | undefined) {
        ...
        if (assetTypeConfig && assetTypeConfig.attributeName && this.asset && this.asset.asset?.attributes && this.asset.asset?.attributes[assetTypeConfig.attributeName] && assetTypeConfig.colours) {
            const currentValue = this.asset.asset?.attributes[assetTypeConfig.attributeName].value;
            overrideOpts = {
        ...
        if (event.eventType === "asset") {
            ...
            switch (assetEvent.cause) {
                ...
                case AssetEventCause.UPDATE:
                    this.onAssetChanged(assetEvent.assetInfoDto);
                    break;
    ...
    protected async onAssetChanged(asset?: AssetInfoDto) {
        if (asset) {
            this.direction = undefined;
            this.displayValue = undefined;

            const locAttr = asset.asset?.attributes ? asset.asset?.attributes[WellknownAttributes.LOCATION] : undefined;
            this._updateLocation(locAttr ? locAttr.value as GeoJSONPoint : null);

            const assetTypeConfig = getMarkerConfigForAssetType(this.config, asset.asset?.type);
            ...
            if (showLabel && asset.asset?.attributes && asset.asset?.attributes[assetTypeConfig?.attributeName]) {
                const attr = asset.asset?.attributes[assetTypeConfig.attributeName];
                const descriptors = AssetModelUtil.getAttributeAndValueDescriptors(asset.asset?.type, attr.name, attr);
                this.displayValue = Util.getAttributeValueAsString(attr, descriptors[0], asset.asset?.type, showUnits, "-");
            }

            if (showDirection) {
                if (asset.asset?.attributes && asset.asset?.attributes[WellknownAttributes.DIRECTION]) {
                    const directionVal = asset.asset?.attributes[WellknownAttributes.DIRECTION].value as number;
