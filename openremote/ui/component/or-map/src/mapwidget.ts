...
export class MapWidget {
    ...
    protected async _forwardGeocode(config: any) {
        const features = [];
        try {
            console.log("Geocode URL:", this._viewSettings!.geocodeUrl);
            let request = this._viewSettings!.geocodeUrl + '/search?q=' + config.query + '&format=geojson&polygon_geojson=1&addressdetails=1&countrycodes=vn';
