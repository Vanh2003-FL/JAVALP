...
import {
    AgentDescriptor,
    Asset,
    AssetDescriptor, AssetDto,
    AssetEvent,
    AssetEventCause, AssetInfoDto,
    AssetModelUtil,
    AssetQuery,
    AssetQueryMatch,
    AssetsEvent,
...
export interface UiAssetTreeNode extends AssetTreeNode {
    ...
    selectedTu: boolean;
    expandable: boolean;
    expandableTu: boolean;
    expanded: boolean;
    expandedTu: boolean;
    parent: UiAssetTreeNode;
    parentTu:UiAssetTreeNode;
    children: UiAssetTreeNode[];
    someChildrenSelected: boolean;
    allChildrenSelected: boolean;
    childrenTu: UiAssetTreeNode[];
    someChildrenSelectedTu: boolean;
    allChildrenSelectedTu: boolean;
...
export interface NodeSelectEventDetailCustom {
    oldNodeTus: UiAssetTreeNode[];
    newNodeTus: UiAssetTreeNode[];
}
...
export class OrAssetTreeRequestSelectionEventCustom extends CustomEvent<Util.RequestEventDetail<NodeSelectEventDetailCustom>> {

    public static readonly NAME = "or-asset-tree-request-selection-custom";

    constructor(request: NodeSelectEventDetailCustom) {
        super(OrAssetTreeRequestSelectionEventCustom.NAME, {
            bubbles: true,
            composed: true,
            detail: {
                allow: true,
...
export class OrAssetTreeSelectionEventCustom extends CustomEvent<NodeSelectEventDetailCustom> {

    public static readonly NAME = "or-asset-tree-selection-custom";

    constructor(detail: NodeSelectEventDetailCustom) {
        super(OrAssetTreeSelectionEventCustom.NAME, {
            bubbles: true,
            composed: true,
            detail: detail,
        });
...
declare global {
    export interface HTMLElementEventMap {
        ...
        [OrAssetTreeSelectionEventCustom.NAME]: OrAssetTreeSelectionEventCustom;
        [OrAssetTreeRequestSelectionEventCustom.NAME]: OrAssetTreeRequestSelectionEventCustom;
...
export const getAssetTypes = async () => {
    ...
    if(response && response.data) {
        return response.data.map(asset => asset.asset?.type!);
    }
}
...
export class OrAssetTree extends subscribe(manager)(LitElement) {
    ...
    public showTu?: {};
    @property({attribute: false})
    public editShowTu?: AssetInfoDto;
    @property({attribute: false})
    public showTu2?: boolean;
    @property({attribute: false})
    ...
    @state()
    protected _nodeTus?: UiAssetTreeNode[];

    protected _loading: boolean = false;
    protected _connected: boolean = false;
    protected _selectedNodes: UiAssetTreeNode[] = [];
    protected _selectedNodeTus: UiAssetTreeNode[] = [];
    ...
    private _latestSelectedTu: UiAssetTreeNode | undefined = undefined;
    ...
    public set selectedNodes(nodes: UiAssetTreeNode[]) {
        this._selectedNodes = nodes ? [...nodes] : [];
        // Update selectedIds from selectedNodes
        this.selectedIds = this._selectedNodes.map((node) => {
            const assetInfo = node.asset as AssetInfoDto;
            const asset = assetInfo?.asset || node.asset as Asset;
            return asset?.id;
        }).filter(id => id !== undefined) as string[];
    }
    public get selectedNodeTus(): UiAssetTreeNode[] {
        return this._selectedNodeTus ? [...this._selectedNodeTus] : [];
    }

    public set selectedNodeTus(nodes: UiAssetTreeNode[]) {
        this.selectedIdTus = nodes.map((node) => node.asset!.id!);
    }
    ...
    protected render() {
        this._nodeTus = this._nodes ? [...this._nodes] : [];
        const filteredData = this._nodeTus?.filter(item => item.asset?.type === "ElectricalCabinetAsset");
        return html`
            <div id="header">
                    ...
                    : ((this._nodes.length === 0 || !this.atLeastOneNodeToBeShown())
                            ...
                            : html`
                                <div id="list-container">
                                    <ol id="list">
                                        ${this.showTu ==="LightAsset" ||this.showTu ==="ElectricalCabinetAsset" ||this.showTu ==="FixedGroupAsset" || this.editShowTu?.asset?.type === "LightAsset" || this.editShowTu?.asset?.type ==="ElectricalCabinetAsset" || this.editShowTu?.asset?.type === "FixedGroupAsset"? this._nodeTus?.map((treeNode:any) => this._treeNodeTemplateShowTu(treeNode, 0)).filter((t:any) => !!t): this._nodes.map((treeNode) => this._treeNodeTemplate(treeNode, 0)).filter(t => !!t)}
    ...
    protected shouldUpdate(_changedProperties: PropertyValues): boolean {
        ...
        if (_changedProperties.has("selectedIdTus")) {
            if (!Util.objectsEqual(_changedProperties.get("selectedIdTus"), this.selectedIdTus)) {
                this._updateSelectedTuNodes();
            }
        }
        ...
        // Handle expandAllNodes change (but not for modal selector)
        if (_changedProperties.has("expandAllNodes") && this.expandAllNodes && this._nodes && this.id !== 'chart-asset-tree') {
    ...
    protected _updateSelectedNodes() {
        ...
        // Use selectedNodes if available, otherwise fall back to selectedIds
        const nodesToSelect = this.selectedNodes && this.selectedNodes.length > 0 
            ? this.selectedNodes 
            : (this.selectedIds ? this.selectedIds.map(id => this._findNodeFromAssetId(id)).filter(n => n !== undefined) as UiAssetTreeNode[] : []);
        
        const selectedIdsSet = new Set(nodesToSelect.map(n => {
            const assetInfo = n.asset as AssetInfoDto;
            const asset = assetInfo?.asset || n.asset as Asset;
            return asset?.id;
        ...
        OrAssetTree._forEachNodeRecursive(this._nodes!, (node) => {
            ...
            if (nodeId && selectedIdsSet.has(nodeId)) {
                actuallySelectedIds.push(nodeId);
            ...
            if (this.checkboxes) {
                ...
                while (parent) {
                    ...
                    if (allChildren.every(c => {
                        const childAssetInfo = c.asset as AssetInfoDto;
                        const childAsset = childAssetInfo?.asset || c.asset as Asset;
                        return childAsset?.id && selectedIdsSet.has(childAsset.id);
                    })) {
                        parent.allChildrenSelected = true;
                    } else if (allChildren.some(c => {
                        const childAssetInfo = c.asset as AssetInfoDto;
                        const childAsset = childAssetInfo?.asset || c.asset as Asset;
                        return childAsset?.id && selectedIdsSet.has(childAsset.id);
                    })) {
    ...
    protected _updateSelectedTuNodes() {
        const actuallySelectedIds: string[] = [];
        const selectedNodes: UiAssetTreeNode[] = [];
        OrAssetTree._forEachNodeRecursive(this._nodeTus!, (node) => {
            if (this.selectedIdTus && this.selectedIdTus.indexOf(node.asset!.id!) >= 0) {
                actuallySelectedIds.push(node.asset!.id!);
                selectedNodes.push(node);
                node.selectedTu = true;

                // Expand every ancestor
                let parent = node.parentTu;
                while (parent) {
                    parent.expanded = true;
                    parent = parent.parentTu;
                }
            } else {
                node.selectedTu = false;
            }
            ...
            if (this.checkboxes) {
                ...
                while (parent) {
                    const allChildren: UiAssetTreeNode[] = [];
                    OrAssetTree._forEachNodeRecursive(parent.childrenTu, (child) => {
                        allChildren.push(child);
                    });
                    parent.someChildrenSelectedTu = false;
                    parent.allChildrenSelectedTu = false;

                    if (allChildren.every(c => actuallySelectedIds.includes(c.asset!.id!))) {
                        parent.allChildrenSelectedTu = true;
                    } else if (allChildren.some(c => actuallySelectedIds.includes(c.asset!.id!))) {
                        parent.someChildrenSelectedTu = true;
                    }
        ...
        this.selectedIdTus = actuallySelectedIds;
        const oldSelection = this._selectedNodeTus;
        this._selectedNodeTus = selectedNodes;
        this.dispatchEvent(new OrAssetTreeSelectionEventCustom({
            oldNodeTus:oldSelection,
            newNodeTus:selectedNodes,
        }));
    ...
    private _findNode(n: UiAssetTreeNode, assetId: string): UiAssetTreeNode | undefined {
        ...
        if (nodeId) {
            if (nodeId === assetId) {
                ...
                n.children.forEach((child: UiAssetTreeNode) => {
                    if (!foundNode) {
                        foundNode = this._findNode(child, assetId);
                    }
                });
                return foundNode;
            }
    ...
    protected _onNodeClicked(evt: MouseEvent | null, node: UiAssetTreeNode | null) {
        ...
        if (isExpander) {
            ...
            if (node && this.config && this.config.select?.types) {
                ...
                const nodeTypeAllowed = nodeType ? this.config.select.types.indexOf(nodeType) >= 0 : false;
                
                // Also allow selection if node has children that match allowed types
                let hasAllowedChildren = false;
                if (!nodeTypeAllowed && node.children && node.children.length > 0 && this.config?.select?.types) {
                    const allowedTypes = this.config.select.types;
                    hasAllowedChildren = node.children.some(child => {
                        const childAssetInfo = child.asset as AssetInfoDto;
                        const childAsset = childAssetInfo?.asset || child.asset as Asset;
                        const childType = childAsset?.type;
                        return childType ? allowedTypes.indexOf(childType) >= 0 : false;
                    });
                }
                
                canSelect = nodeTypeAllowed || hasAllowedChildren;
            }
            ...
            if (node) {
                ...
                const asset = assetInfo?.asset || node.asset as Asset;
                const nodeId = asset?.id;
                const index = nodeId ? this.selectedNodes.findIndex(n => {
                    const nAssetInfo = n.asset as AssetInfoDto;
                    const nAsset = nAssetInfo?.asset || n.asset as Asset;
                    return nAsset?.id === nodeId;
                }) : -1;
                let select = true;
            ...
            const event = new OrAssetTreeRequestSelectionEvent({
                oldNodes: this.selectedNodes,
                newNodes: selectedNodes,
            });
            Util.dispatchCancellableEvent(this, event).then((detail) => {
                if (detail.allow) {
                    this.selectedNodes = detail.detail.newNodes;
                    // Update selectedIds from selectedNodes
                    this.selectedIds = this.selectedNodes.map(n => {
                        const assetInfo = n.asset as AssetInfoDto;
                        const asset = assetInfo?.asset || n.asset as Asset;
                        return asset?.id;
                    }).filter(id => id !== undefined) as string[];
    ...
    protected _onNodeClickedShowTu(evt: MouseEvent | null, node: UiAssetTreeNode | null) {
        if (evt && evt.defaultPrevented) {
            return;
        }
        if (evt) {
        ...
        if (isExpander) {
            ...
            if (node) {
                const index = this.selectedNodeTus.indexOf(node);
                let select = true;
                let deselectOthers = true;
                const multiSelect = !this._isReadonly() && (!this.config || !this.config.select || !this.config.select.multiSelect);
                ...
                if (this.checkboxes || (multiSelect && evt && (evt.ctrlKey || evt.shiftKey || evt.metaKey))) {
                    deselectOthers = false;
                    if (index >= 0 && this.selectedIdTus && this.selectedIdTus.length > 1) {
                        select = false;
                    }
                }
                ...
                // handle selected state
                if (isParentCheckbox) {
                    selectedNodes = [...this.selectedNodeTus];

                    const childNodes: UiAssetTreeNode[] = [];
                    OrAssetTree._forEachNodeRecursive(node.childrenTu, (childNode) => {
                        let canSelectChild = true;
                        // If not of required asset type, cancel selection of the child
                        if(childNode && this.config?.select?.types) {
                            canSelectChild = this.config.select.types.indexOf(childNode.asset!.type!) >= 0;
                    ...
                    // based on multiple-box already selected, remove or add to array of selected nodes
                    selectedNodes = (!node.allChildrenSelectedTu)
                        ? selectedNodes.concat(childNodes)
                        : selectedNodes.filter(n => !childNodes.map(cn => cn.asset!.id).includes(n.asset!.id));

                } else if (deselectOthers) {
                    this._latestSelectedTu = Object.assign({}, node);
                    selectedNodes = [node];
                } else if (select) {
                    if (index < 0) {
                        if (evt && evt.shiftKey) {
                            let hierarchy: string[] = [];
                            this._nodeTus?.forEach((n: UiAssetTreeNode) => {
                                hierarchy = hierarchy.concat(this._buildPaths(n));
                            });

                            if (this._latestSelectedTu && this._latestSelectedTu.asset && this._latestSelectedTu.asset.id && node.asset && node.asset.id) {
                                let latestSelectedAssetId: string = this._latestSelectedTu.asset.id;
                                let newlySelectedAssetId: string = node.asset.id;
                    ...
                    selectedNodes = [...this.selectedNodeTus];
                    if (selectedNodes.length === 1) {
                        this._latestSelectedTu = undefined;
                    }
                    selectedNodes.splice(index, 1);
                }
            }
            Util.dispatchCancellableEvent(this, new OrAssetTreeRequestSelectionEventCustom({
                newNodeTus:selectedNodes,
                oldNodeTus:this.selectedNodeTus
            })).then((detail) => {
                if (detail.allow) {
                    this.selectedNodeTus = detail.detail.newNodeTus;
                    this._updateSelectedTuNodes();
                    this.requestUpdate();
                }
            });
    ...
    protected async getMatcherFromQuery(): Promise<((asset: Asset) => boolean)> {
        ...
        try {
            ...
            foundAssetIds = response.data.map((asset: AssetInfoDto) => asset.asset?.id!);
        } catch (e) {
    ...
    protected async _onCopyClicked() {
        ...
        try {
            ...
            const asset = JSON.parse(JSON.stringify(response.data)) as AssetInfoDto;
            asset.asset!.name += " copy";
            delete asset.asset?.id;
            delete asset.asset?.path;
            delete asset.asset?.createdOn;
            delete asset.asset?.version;
            ...
            console.error("Failed to copy asset2222", e);
    ...
    protected _onAddClicked() {
        ...
        dialog = showDialog(new OrMwcDialog()
            ...
            .setContent(html`
                <or-add-asset-dialog id="add-panel" .config="${this.config}" .agentTypes="${agentTypes}" .assetTypes="${assetTypes}"  .parent="${parent}"  @selectedTypeCustom="${onAddChanged}"></or-add-asset-dialog>
            `)
            .setActions([
                {
                    actionName: "cancel",
                ...
                {
                    actionName: "add",
                    content: html`<or-mwc-input id="add-btn" class="button" .type="${InputType.BUTTON}" label="add" ></or-mwc-input>`,
                    action: () => {

                        const addAssetDialog = dialog.shadowRoot!.getElementById("add-panel") as OrAddAssetDialog;
                        const descriptor = addAssetDialog.selectedTypeCustom;
                        const attributeItem = addAssetDialog.atributeItem;
                        const selectedOptionalAttributes = addAssetDialog.selectedAttributes;
                        const name = addAssetDialog.name.trim();
                        const assetCode = addAssetDialog.assetCode;
                        const cabinetAssetCode = addAssetDialog.cabinetAssetCode;
                        const parent = addAssetDialog.parent;
                        const newparent  = addAssetDialog.newParent
                        const active = addAssetDialog.active
                        const iscontrolcabinetid = addAssetDialog.isControlByCabinet
                        if (!descriptor) {
                            return;
                        }

                        const asset: AssetInfoDto = {
                            asset:{
                                name: name,
                            ...
                            assetInfo:{
                                assetCode:assetCode,
                                cabinetAssetCode:cabinetAssetCode,
                                cabinetId:newparent?.id,
                                active:active,
                                isControlByCabinet:iscontrolcabinetid
                            }
                        };

                        // Construct attributes
                        const assetTypeInfo = AssetModelUtil.getAssetTypeInfo(descriptor.value!);
                        if (!assetTypeInfo) {
                            return;
                        }
                        ...
                        //             asset.asset!.attributes![attributeDescriptor.name!] = {
                        ...
                        if (attributeItem) {
                            ...
                            attributeItem
                                .forEach((attributeDescriptor) => {
                                    asset.asset!.attributes![attributeDescriptor.attrCodeName!] = {
                                        name: attributeDescriptor.attrCodeName,
                                        type: attributeDescriptor.attrDataType,
                                        // meta: attributeDescriptor.meta ? {...attributeDescriptor.meta} : undefined
                                    } as Attribute<any>;
                                });
                        }
                        ...
                        //         asset.asset!.attributes![attribute.name!] = {
                        //             name: attribute.name,
                        //             type: attribute.type,
                        ...
                        if (this.selectedIds) {
                            asset.asset!.parentId = parent ? parent.id : undefined;
                        }
        ...
        if (event.eventType === "asset" && this._nodes && this._nodes.length > 0) {
            ...
            if (assetEvent.cause === AssetEventCause.UPDATE
                && !(assetEvent.updatedProperties!.includes("name")
                    ...
                    || assetEvent.updatedProperties!.includes("assetcode"))
            ) {
                return;
            }
    ...
    protected _buildTreeNodes(assets: Asset[], sortFunction: (a: UiAssetTreeNode, b: UiAssetTreeNode) => number) {
        if (!assets || assets.length === 0) {
            ...
            // Only restore expanded state if NOT in modal selector
            if (this.id !== 'chart-asset-tree') {
                ...
                // In modal selector, clear expanded state to start fresh
                this._expandedNodes = [];
                OrAssetTree._forEachNodeRecursive(this._nodes, (node) => {
                    node.expanded = false;
                });
        ...
        if (this.selectedIdTus && this.selectedIdTus.length > 0) {
            this._updateSelectedTuNodes();

        }

        // Only expand all nodes if expandAllNodes is true AND not in modal selector
        // Modal selector should only expand root level (handled in _treeNodeTemplate)
        if (this.expandAllNodes && this._nodes && this.id !== 'chart-asset-tree') {
            OrAssetTree._forEachNodeRecursive(this._nodes, (node) => {
                if (node.expandable && node.children && node.children.length > 0) {
                    node.expanded = true;
                }
            });
            // Request update to reflect the expanded state
            this.requestUpdate();
        }
    }
    ...
    public _onDragLeaveTu(ev: any): void {
        let currentElement = ev.currentTarget as HTMLElement;
    ...
    protected _treeNodeTemplate(treeNode: UiAssetTreeNode, level: number): TemplateResult | string | undefined {
        const assetInfo = treeNode.asset as AssetInfoDto;
        const asset = assetInfo?.asset || treeNode.asset as Asset;
        ...
        // DO NOT auto-expand in modal selector - let user manually expand/collapse using expander icon
        // Only handle expandAllNodes prop if set (and not in modal selector)
        if (this.expandAllNodes && this.id !== 'chart-asset-tree' && treeNode.expandable && treeNode.children && treeNode.children.length > 0 && !treeNode.expanded) {
            treeNode.expanded = true;
        }
        ...
        return  html`
            <li class="asset-list-element" ?data-selected="${treeNode.selected}" ?data-expanded="${treeNode.expanded}" @click="${(evt: MouseEvent) => this._onNodeClicked(evt, treeNode)}">
                ...
                <div class="node-container draggable" node-asset-id="${treeNode.asset ? treeNode.asset.id : ''}" draggable="true" @dragleave=${(ev: DragEvent) => { this._onDragLeave(ev) }} @dragenter="${(ev: DragEvent) => this._onDragEnter(ev)}" @dragstart="${(ev: DragEvent) => this._onDragStart(ev)}" @dragend="${(ev: DragEvent) => this._onDragEnd(ev)}" @dragover="${(ev: DragEvent) => this._onDragOver(ev)}">
                    <div class="node-name" style="padding-left: ${level * 22}px;">
                        <div class="expander" ?data-expandable="${treeNode.expandable}" style="display: inline-flex; width: 36px; min-width: 36px; height: 40px; min-height: 40px; align-items: center; justify-content: center;" @click="${(e: MouseEvent) => { if (treeNode.expandable) { e.stopPropagation(); this._toggleExpander(e.target as HTMLElement, treeNode); } }}"></div>
                        ${getAssetDescriptorIconTemplate(descriptor, undefined, undefined, (filterColorForNonMatchingAsset ? 'd3d3d3' : undefined))}
                        <span class="asset-name-text" style="color: ${filterColorForNonMatchingAsset ? '#d3d3d3' : '#000'} !important; flex: 1 1 auto !important; min-width: 0 !important; overflow: visible !important; text-overflow: clip !important; white-space: normal !important; display: inline-block !important; margin-right: 8px !important; visibility: visible !important; opacity: 1 !important; font-size: 14px !important; width: auto !important; max-width: 100% !important;">${asset?.name || 'NO NAME'}</span>
                        ${this.checkboxes ? html`
                                    <span class="mdc-list-item__graphic" @click="${(e: MouseEvent) => { e.stopPropagation(); this._onNodeClicked(e, treeNode); }}">
                                ${treeNode.expandable
                                        ? html`<div class="mdc-checkbox" @click="${(e: MouseEvent) => { e.stopPropagation(); this._onNodeClicked(e, treeNode); }}">
                                            <or-icon class="mdc-checkbox--parent" icon="${parentCheckboxIcon}"></or-icon>
                                        </div>`
                                        : ``}
                                <div class="mdc-checkbox" @click="${(e: MouseEvent) => { e.stopPropagation(); this._onNodeClicked(e, treeNode); }}">
    ...
    protected _treeNodeTemplateShowTu(treeNode: UiAssetTreeNode, level: number): TemplateResult | string | undefined {
        const assetInfo = treeNode.asset as AssetInfoDto;
        const asset = assetInfo?.asset || treeNode.asset as Asset;
        const descriptor = AssetModelUtil.getAssetDescriptor(asset?.type!);
        let parentCheckboxIcon;
        if (treeNode.allChildrenSelectedTu) {
            parentCheckboxIcon = 'checkbox-multiple-marked';
        } else if (treeNode.someChildrenSelectedTu) {
            parentCheckboxIcon = 'checkbox-multiple-marked-outline';
        } else {
            parentCheckboxIcon = 'checkbox-multiple-blank-outline';
        }
        ...
        return html
            `<li class="asset-list-element" ?data-selected="${treeNode.selectedTu}" ?data-expanded="${treeNode.expanded}" @click="${(evt: MouseEvent) => this._onNodeClickedShowTu(evt, treeNode)}">
                <div class="in-between-element" node-asset-id="${treeNode.parentTu ? (treeNode.parentTu.asset ? treeNode.parentTu.asset.id : '' ) : undefined}" @dragleave=${(ev: DragEvent) => { this._onDragLeaveTu(ev) }} @dragenter="${(ev: DragEvent) => this._onDragEnter(ev)}" @dragend="${(ev: DragEvent) => this._onDragEnd(ev)}" @dragover="${(ev: DragEvent) => this._onDragOver(ev)}"></div>
                <div class="node-container draggable" node-asset-id="${treeNode.asset ? treeNode.asset.id : ''}" draggable="true" @dragleave=${(ev: DragEvent) => { this._onDragLeaveTu(ev) }} @dragenter="${(ev: DragEvent) => this._onDragEnter(ev)}" @dragstart="${(ev: DragEvent) => this._onDragStart(ev)}" @dragend="${(ev: DragEvent) => this._onDragEnd(ev)}" @dragover="${(ev: DragEvent) => this._onDragOver(ev)}">
                    <div class="node-name" style="padding-left: ${level * 22}px;">
                        <div class="expander" ?data-expandable="${treeNode.expandable}"></div>
                        ${getAssetDescriptorIconTemplate(descriptor, undefined, undefined, (filterColorForNonMatchingAsset ? 'd3d3d3' : undefined))}
                        <span class="asset-name-text" style="color: ${filterColorForNonMatchingAsset ? '#d3d3d3' : '#000'} !important; flex: 1 1 auto !important; min-width: 0 !important; overflow: visible !important; text-overflow: clip !important; white-space: normal !important; display: inline-block !important; margin-right: 8px !important; visibility: visible !important; opacity: 1 !important; font-size: 14px !important; width: auto !important; max-width: 100% !important;">${asset?.name || 'NO NAME'}</span>
                        ${this.checkboxes ? html`
                                    <span class="mdc-list-item__graphic" @click="${(e: MouseEvent) => { e.stopPropagation(); this._onNodeClickedShowTu(e, treeNode); }}">
                                ${treeNode.expandable
                                        ? html`<div class="mdc-checkbox" @click="${(e: MouseEvent) => { e.stopPropagation(); this._onNodeClickedShowTu(e, treeNode); }}">
                                            <or-icon class="mdc-checkbox--parent" icon="${parentCheckboxIcon}"></or-icon>
                                        </div>`
                                        : ``}
                                <div class="mdc-checkbox" @click="${(e: MouseEvent) => { e.stopPropagation(); this._onNodeClickedShowTu(e, treeNode); }}">
                                    ${treeNode.selectedTu ? html`<or-icon icon="checkbox-marked"></or-icon>`: html`<or-icon icon="checkbox-blank-outline"></or-icon>`}
                                </div>
                            </span>`
                                : ``}
                    </div>
                ...
                <ol>
                    ${!treeNode.children || (treeNode.expandable && !treeNode.expandable)  ? `` : treeNode.children.map((childNode) => this._treeNodeTemplateShowTu(childNode, level + 1)).filter(t => !!t)}
                </ol>
            </li>
