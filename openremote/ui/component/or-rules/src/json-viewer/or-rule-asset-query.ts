...
export class OrRuleAssetQuery extends translate(i18next)(LitElement) {
    ...
    @property({type: Object})
    public assetProvider!: (type: string) => Promise<AssetInfoDto[] | undefined>
    ...
    protected attributePredicateEditorTemplate(assetTypeInfo: AssetTypeInfo, asset: AssetInfoDto | undefined, attributePredicate: AttributePredicate) {
        ...
        let attribute = asset && asset.asset?.attributes && attributeName ? asset.asset?.attributes[attributeName] : undefined;
        let attributes: [string, string][] = [];
        const descriptors = AssetModelUtil.getAttributeAndValueDescriptors(asset ? asset.asset?.type : assetDescriptor.name, attribute || attributeName, attribute);

        if (asset && asset.asset?.attributes) {
            attributes = Object.values(asset.asset?.attributes)
                // .filter((item) => ["brightness", "colourTemperature", "status"].includes(item.name))
                ...
                .map((attr) => {
                    ...
                    const label = Util.getAttributeLabel(attr, descriptors[0], asset.asset?.type, false);
                    return [attr.name!, label];
                });
            console.log('attributes',asset.asset?.attributes)
        } else {
    ...
    protected attributePredicateValueEditorTemplate(assetDescriptor: AssetDescriptor, asset: AssetInfoDto | undefined, attributePredicate: AttributePredicate) {
        ...
        const attribute = asset && asset.asset?.attributes && attributeName ? asset.asset?.attributes[attributeName] : undefined;
            ...
            <div class="attribute-group">
                ...
                `, () => {
                    ...
                    if(this._assets!.length <= 25) {
                        idOptions.push(...this._assets!.map((asset) => [asset.asset?.id!, asset.asset?.name!] as [string, string]));
                        
                    } else {
                        searchProvider = async (search?: string) => {
                            if(search) {
                                return this._assets!.filter((asset) => asset.asset?.name?.toLowerCase().includes(search.toLowerCase())).map((asset) => [asset.asset?.id!, asset.asset?.name!] as [string, string]);
                            } else if (this._assets!.length <= 100) {
                                idOptions.push(...this._assets!.map((asset) => [asset.asset?.id!, asset.asset?.name!] as [string, string]));
                                return idOptions;
                            } else {
                                const asset = this._assets?.find((asset) => asset.asset?.id == idValue);
                                if(asset && idOptions.find(([id, _value]) => id == asset.asset?.id) == undefined) {
                                    idOptions.push([asset.asset?.id!, asset.asset?.name!]); // add selected asset if there is one.
                                }
                                return idOptions;
                            }
                        }
                        ...
                        <div class="attributes">
                            ${this.query.attributes && this.query.attributes.items ? this.query.attributes.items.map((attributePredicate, index) => {
                                return html`
                                    ...
                                    <div class="attribute">
                                        <div>
                                    ${this.attributePredicateEditorTemplate(assetTypeInfo, idValue !== "*" ? this._assets!.find((asset) => asset.asset?.id === idValue) : undefined, attributePredicate)}
                                        </div>
