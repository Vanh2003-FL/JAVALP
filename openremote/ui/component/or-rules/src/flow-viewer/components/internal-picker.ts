...
export class InternalPicker extends translate(i18next)(LitElement) {
    ...
    @property({ type: Object }) private selectedAsset!: AssetInfoDto;
    ...
    private async setSocketTypeDynamically(value: AttributeInternalValue) {
        ...
        if (results[0].asset?.attributes == null) { return; }
        try {
            const relevantAttribute = results[0].asset?.attributes[value.attributeName!];
    ...
    private get assetAttributeInput(): TemplateResult {
        ...
        const openDialog = () => {
            ...
            if (this.selectedAsset && this.selectedAsset.asset?.id) {
                _selectedAssets = [ this.selectedAsset.asset?.id ];
            }
        ...
        if (this.selectedAsset && this.selectedAsset.asset?.attributes && this.internal?.value?.attributeName)
        {
            const attrName = this.internal?.value?.attributeName;
            const attributeDescriptor = AssetModelUtil.getAttributeDescriptor(attrName, this.selectedAsset.asset?.type!);
            let attr = this.selectedAsset.asset?.attributes[attrName];
            if (attr) {
                selectedAttrLabel = Util.getAttributeLabel(attr, attributeDescriptor, this.selectedAsset.asset?.type!, true);
            }
        }

        const descriptor = this.selectedAsset ? AssetModelUtil.getAssetDescriptor(this.selectedAsset!.asset?.type!) : undefined;
            ...
            ${(this.selectedAsset ? 
                    html`<div class="attribute-label attribute-label-white selected-asset-container" @click="${() => openDialog()}">
                        ...
                        <div class="selected-asset-label">
                            <div class="asset">${this.selectedAsset.asset?.name}</div>
