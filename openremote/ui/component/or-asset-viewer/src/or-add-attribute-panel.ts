...
import {Asset, Attribute, AssetModelUtil, AssetInfoDto} from "@openremote/model";
...
export class OrAddAttributePanel extends LitElement {
    ...
    protected shouldUpdate(_changedProperties: PropertyValues) {
        if (_changedProperties.has("asset")) {
            ...
            this.attributeTypes = (AssetModelUtil.getAssetTypeInfo(this.asset.asset!.type!)?.attributeDescriptors || [])
                .filter((descriptor) => !this.asset.asset!.attributes![descriptor.name!])
                .sort(Util.sortByString((descriptor) => descriptor.name!))
                .map((descriptor) => {
                    return [
                        descriptor.name!,
                        Util.getAttributeLabel(undefined, descriptor, this.asset.asset!.type, false)
                    ]
                });
            this.attributeTypes = [["@custom", i18next.t("custom")], ...this.attributeTypes];

            this.attributeValueTypes = (AssetModelUtil.getAssetTypeInfo(this.asset.asset!.type!)?.valueDescriptors || [])
    ...
    protected render() {
        console.log('attributetype',this.asset)
    ...
    protected onTypeChanged(name: string) {
        if (name === "@custom") {
            ...
            const descriptor = AssetModelUtil.getAttributeDescriptor(name, this.asset.asset!.type!)!;
            this.attribute = {};
