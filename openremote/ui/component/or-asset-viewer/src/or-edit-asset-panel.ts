...
import {Asset, Attribute, NameValueHolder, AssetModelUtil, AssetDto, AssetInfoDto} from "@openremote/model";
...
const style = css`
    ...
    #parent-edit-wrapper > #property-cabinetId {
        width: 100%;
    }
...
export class OrEditAssetPanel extends LitElement {
    ...
    public attributeUpdated(attributeName: string) {
        ...
        console.log('attrbuteName', attributeName)
    ...
    protected render() {
        console.log('ban dang o man chinh sua 2')
        const updatePublicRead = (publicRead: boolean) => {
            if (this.asset.asset) {
                this.asset.asset.accessPublicRead = publicRead;
            }
            this._onModified();
        };
        const updateAssetCode = (assetCode: string) => {
            if (this.asset.assetInfo) {
                this.asset.assetInfo.assetCode = assetCode;
            }
            console.log('a')
            this._onModified();
        };
        const updateNemaCode = (nemaCode: string) => {
            if (this.asset.assetInfo) {
                this.asset.assetInfo.cabinetAssetCode = nemaCode;
            }

            this._onModified();
        };
        const updateQLBT = (qlbt: boolean) => {
            if (this.asset.assetInfo) {
                this.asset.assetInfo.isControlByCabinet = qlbt;
            }

            this._onModified();
        };
        const updateTTHD = (active: boolean) => {
            if (this.asset.assetInfo) {
                this.asset.assetInfo.active = active;
            }
        ...
        const properties: TemplateResult[] = [
            getField("type", undefined, getPropertyTemplate(this.asset, "type", this, undefined, undefined, {
                readonly: true,
            ...
            html`
                    ...
                    <div style="display: flex;flex-direction: column;width: 100%">
                        <div @or-mwc-input-changed="${(ev: OrInputChangedEvent) => updateAssetCode(ev.detail.value)}">
                            ${getField("assetCode", undefined, getPropertyTemplate(this.asset, "assetCode", this, undefined, undefined, {
                                readonly: false,
                                label: i18next.t("assetCode")
                            }))}
                        </div>
                        <div @or-mwc-input-changed="${(ev: OrInputChangedEvent) => updateNemaCode(ev.detail.value)}">
                            ${getField("cabinetAssetCode", undefined, getPropertyTemplate(this.asset, "cabinetAssetCode", this, undefined, undefined, {
                                readonly: false,
                                label: i18next.t("cabinetAssetCode")
                            }))}
                        </div>
                    </div>
                    ${getField("cabinetId", undefined, this._getShowTuTemplate())}
                <div @or-mwc-input-changed="${(ev: OrInputChangedEvent) => updatePublicRead(ev.detail.value as boolean)}">
                    ${getField("accessPublicRead", undefined, getPropertyTemplate(this.asset, "accessPublicRead", this, undefined, undefined, {
                        readonly: false,
                        label: i18next.t("accessPublicRead")
                    }))}
                </div>
               
                    <div @or-mwc-input-changed="${(ev: OrInputChangedEvent) => updateQLBT(ev.detail.value)}">
                        ${getField("isControlByCabinet", undefined, getPropertyTemplate(this.asset, "isControlByCabinet", this, undefined, undefined, {
                            readonly: false,
                            label: i18next.t("isControlByCabinet")
                        }))}
                    </div>
                    <div @or-mwc-input-changed="${(ev: OrInputChangedEvent) => updateTTHD(ev.detail.value)}">
                        ${getField("active", undefined, getPropertyTemplate(this.asset, "active", this, undefined, undefined, {
                            readonly: false,
        ...
        this.attributeTemplatesAndValidators = !this.asset.asset?.attributes ? [] : Object.entries(this.asset.asset?.attributes!).sort(Util.sortByString(([name, attribute]) => name.toUpperCase())).map(([name, attribute]) => {
            attribute.name = name;
            return this._getAttributeTemplate(this.asset.asset?.type!, attribute as Attribute<any>);
        })
                    ...
                    <tr class="mdc-data-table__row">
                        <td colspan="4">
                            <div class="item-add-attribute">
                                <or-mwc-input .type="${InputType.BUTTON}" label="addAttribute" icon="plus"
                                              @or-mwc-input-changed="${() => this._addAttribute()}"></or-mwc-input>
                            </div>
                        </td>
                    </tr>
                    </tbody>
    ...
    protected _getAttributeTemplate(assetType: string, attribute: Attribute<any>): TemplateAndValidator {
        ...
        const deleteAttribute = () => {
            delete this.asset.asset?.attributes![attribute.name!];
            this._onModified();
        };
        ...
        const template = html`
            <tr class="mdc-data-table__row">
                <td class="padded-cell mdc-data-table__cell expander-cell">
                    ...
                    <span><or-translate value="${attribute.name}"></or-translate></span></td>
                <td class="padded-cell mdc-data-table__cell">
                    <or-translate value = "${Util.getValueDescriptorLabel(attribute.type!)}"></or-translate>
                </td>
                <td class="padded-cell overflow-visible mdc-data-table__cell">
                    <or-attribute-input ${ref(attributeInputRef)}
                                        ...
                                        .readonly="${false}" .attribute="${attribute}"
                ...
                <td class="padded-cell mdc-data-table__cell actions-cell">${canDelete ? html`
                    <or-mwc-input type="${InputType.BUTTON}" icon="delete" @click="${deleteAttribute}">` : ``}
                </td>
            </tr>
                ...
                <td colspan="4">
                    <div class="meta-item-container">
                        <div>
                            ...
                            <div class="item-add">
                                <or-mwc-input .type="${InputType.BUTTON}" label="addMetaItems" icon="plus"
                                              @or-mwc-input-changed="${() => this._addMetaItems(attribute)}"></or-mwc-input>
                            </div>
                        </div>
                    </div>
                </td>
    ...
    protected _getMetaItemTemplate(attribute: Attribute<any>, metaItem: NameValueHolder<any>): TemplateAndValidator {
        ...
        if (!valueDescriptor) {
            ...
            content = html`
                <or-mwc-input
                        @or-mwc-input-changed="${(ev: OrInputChangedEvent) => this._onMetaItemModified(attribute, metaItem, ev.detail)}"
                        .type="${InputType.JSON}" .value="${metaItem.value}"></or-mwc-input>`;
        } else {
            const options: ValueInputProviderOptions = {
                label: Util.getMetaLabel(metaItem, descriptor!, this.asset.asset?.type!, true),
                resizeVertical: true
            };
            const standardInputProvider = getValueHolderInputTemplateProvider(this.asset.asset?.type!, metaItem, descriptor, valueDescriptor, (detail) => this._onMetaItemModified(attribute, metaItem, detail), options);
            let provider = jsonFormsInputTemplateProvider(standardInputProvider)(this.asset.asset?.type!, metaItem, descriptor, valueDescriptor, (detail) => this._onMetaItemModified(attribute, metaItem, detail), options);
    ...
    protected _addAttribute() {
        ...
        const isDisabled = (attribute: Attribute<any>) => {
            return !(attribute && attribute.name && !asset.asset?.attributes![attribute.name] && AssetNameRegex.test(attribute.name) && attribute.type);
        }
        ...
        const dialog = showDialog(new OrMwcDialog()
            .setContent(html`
                <or-add-attribute-panel .asset="${asset}"
                                        @or-add-attribute-panel-attribute-changed="${(ev: OrAddAttributePanelAttributeChangedEvent) => onAttributeChanged(ev.detail)}"></or-add-attribute-panel>
            `)
            .setStyles(html`
                <style>
                    .mdc-dialog__surface {
            ...
            .setActions([
                ...
                },
                {
                    ...
                    action: () => {
                        if (attr) {
                            this.asset.asset!.attributes![attr.name!] = attr;
                            // this.asset.asset?.attributes![attr.name!] = attr;
                            this._onModified();
                        }
                    },
                    content: html`
                        <or-mwc-input id="add-btn" .type="${InputType.BUTTON}" disabled label="add"
    ...
    protected _addMetaItems(attribute: Attribute<any>) {
        ...
        const assetTypeInfo = AssetModelUtil.getAssetTypeInfo(this.asset!.asset?.type!);
        console.log('metaItem2',AssetModelUtil.getAssetTypeInfo(this.asset!.asset?.type!))
        ...
        const metaItemList: (ListItem | null)[] = assetTypeInfo.metaItemDescriptors.map((metaName) => AssetModelUtil.getMetaItemDescriptor(metaName)!)
            ...
            .map((descriptor) => {
                return {
                    text: Util.getMetaLabel(undefined, descriptor, this.asset!.asset?.type!, true),
                    value: descriptor.name!,
                    translate: false
                };
            }).sort(Util.sortByString((item) => item.text));

        console.log('metaItem22',this.asset!)
        const dialog = showDialog(new OrMwcDialog()
            .setContent(html`
                <div id="meta-creator">
                    <or-mwc-list id="meta-creator-list" .type="${ListType.MULTI_CHECKBOX}"
                                 .listItems="${metaItemList}"></or-mwc-list>
                </div>
            `)
            .setStyles(html`
                <style>
    ...
    protected _getParentTemplate() {
        ...
        const setParent = () => {
            ...
            this.asset.asset!.parentId = assetTree.selectedIds!.length === 1 ? assetTree.selectedIds![0] : undefined;
            // Need to update the assets path as well
            console.log('this.asset.asset?.id',this.asset.asset?.id)
            const path = [this.asset.asset?.id!];
            ...
            this.asset.asset!.path = path;
            this._onModified();
        };

        const clearParent = () => {
            this.asset.asset!.parentId = undefined;
            this.asset.asset!.path = [this.asset.asset?.id!];
            this._onModified();
        };
        ...
        const parentSelector = (node: UiAssetTreeNode) => node.asset!.id !== this.asset.asset?.id;
        console.log('parentSelector',this.asset!.asset?.parentId)
        ...
        const dialogContent = html`
            <or-asset-tree id="parent-asset-tree" disableSubscribe readonly
                           .selectedIds="${this.asset!.asset?.parentId ? [this.asset.asset?.parentId] : []}"
                           @or-asset-tree-request-select="${blockEvent}"
                           @or-asset-tree-selection-changed="${blockEvent}"
                           .selector="${parentSelector}"></or-asset-tree>`;
        ...
        return html`
            <div id="parent-edit-wrapper">
                ${getPropertyTemplate(this.asset, "parentId", this, undefined, undefined, {
                    readonly: true,
                    label: i18next.t("parent")
                })}
                <or-mwc-input id="change-parent-btn" type="${InputType.BUTTON}" outlined label="edit"
                              @or-mwc-input-changed="${openDialog}"></or-mwc-input>
            </div>
        `;
    }

    protected _getShowTuTemplate() {
        const viewer = this;
        let dialog: OrMwcDialog;

        const setParent = () => {
            const assetTree = dialog.shadowRoot!.getElementById("parent-asset-tree2") as OrAssetTree;
            this.asset.assetInfo!.cabinetId = assetTree.selectedIdTus!.length === 1 ? assetTree.selectedIdTus![0] : undefined;
            // console.log('this.asset.assetInfo!.cabinetId',this.asset.assetInfo!.cabinetId)
            // console.log('this.asset.assetInfo!.parentId',this.asset.asset!.parentId)
            // // Need to update the assets path as well
            // const path = [this.asset.assetInfo?.cabinetId!];
            // let parentNode = assetTree.selectedNodeTus[0];
            // while (parentNode !== undefined) {
            //     path.unshift(parentNode.asset!.id!);
            //     parentNode = parentNode.parentTu;
            // }
            // this.asset.asset!.path = path;
            this._onModified();
        };
        ...
        const clearParent = () => {
            this.asset.assetInfo!.cabinetId = "";
            // console.log('this.asset.assetInfo!.cabinetId',this.asset.assetInfo!.cabinetId)
            // console.log('this.asset.assetInfo!.cabinetId2',this.asset.asset!.parentId)
            // this.asset.asset!.path = [this.asset.assetInfo!.cabinetId!];
            this._onModified();
        };
        ...
        // Prevent change event from bubbling up as it will affect any ancestor listeners that are interested in a different asset tree
        const dialogContent = html`<or-asset-tree id="parent-asset-tree2" disableSubscribe .editShowTu="${this.asset}"  readonly .selectedIdTus="${this.asset!.assetInfo!.cabinetId ? [this.asset.assetInfo!.cabinetId] : []}" @or-asset-tree-request-select="${blockEvent}" @or-asset-tree-selection-changed="${blockEvent}" .selector="${parentSelector}"></or-asset-tree>`;
        ...
        return html`
            <div id="parent-edit-wrapper">
                ${getPropertyTemplate(this.asset, "cabinetId", this, undefined, undefined, {readonly: true, label: i18next.t("cabinetId")})}
