...
import {
    AgentDescriptor,
    Asset, AssetDescriptor, AssetDto,
...
interface AssetInfo {
    ...
    childAssets?: AssetInfoDto[];
    viewerConfig: AssetViewerConfig;
    modified: boolean;
    attributeTemplateMap: {[attrName: string]: TemplateResult};
}

export type PanelViewProvider = (asset: AssetInfoDto, attributes: { [index: string]: Attribute<any> }, panelName: string, hostElement: LitElement, viewerConfig: AssetViewerConfig, panelConfig: PanelConfigUnion) => TemplateResult | undefined;
export type PropertyViewProvider = (asset: AssetInfoDto, property: string, value: any, hostElement: LitElement, viewerConfig: AssetViewerConfig, panelConfig: PanelConfigUnion) => TemplateResult | undefined;
export type AttributeViewProvider = (asset: AssetInfoDto, attribute: Attribute<any>, hostElement: LitElement, viewerConfig: AssetViewerConfig, panelConfig: PanelConfigUnion) => TemplateResult | undefined;
...
export class OrAssetViewerRequestSaveEvent extends CustomEvent<Util.RequestEventDetail<AssetInfoDto>> {

    public static readonly NAME = "or-asset-viewer-request-save";

    constructor(asset: AssetInfoDto) {
...
function getPanelContent(id: string, assetInfo: AssetInfo, hostElement: LitElement, viewerConfig: AssetViewerConfig, panelConfig: PanelConfigUnion): TemplateResult | undefined {
    ...
    const asset = assetInfo.assetInfoDto;
    console.log('panelConfig')
    // See if config has a custom way for rendering this panel
    if (viewerConfig.panelViewProvider) {
        const template = viewerConfig.panelViewProvider(asset, asset.asset?.attributes!, id, hostElement, viewerConfig, panelConfig);
        if (template) {
            return template;
        }
    }
    ...
    if (panelConfig.type === "info") {
        ...
        const includedAttributes = getIncludedAttributes(asset.asset?.attributes!, infoConfig);
        ...
        includedAttributes.forEach((attribute) => {
            ...
            if (itemConfig.label === undefined) {
                ...
                itemConfig.label = Util.getAttributeLabel(attribute, descriptors[0], asset.asset?.type, true);
            }
            itemConfig.priority = itemConfig.priority || 0;
            items.push({
                item: attribute,
    ...
    if (panelConfig.type === "setup") {
        ...
        const descriptor = AssetModelUtil.getAssetDescriptor(asset.asset?.type) as AgentDescriptor;

        if (!descriptor || !asset.asset?.id || descriptor.descriptorType !== "agent" || (!descriptor.assetDiscovery && !descriptor.assetImport)) {
            return;
        }
        ...
        const discoverAssets = () => {
            ...
            manager.rest.api.AgentResource.doProtocolAssetDiscovery(asset.asset?.id!)
                .then(response => {
            ...
            if (fileInputElem) {
                ...
                reader.onload = () => {
                    if (!reader.result) {
                        ...
                        manager.rest.api.AgentResource.doProtocolAssetImport(asset.asset?.id!, fileInfo, undefined, {timeout: 30000})
                            .then(response => {
                                if (response.status !== 200) {
                                    showSnackbar(undefined, "somethingWentWrong", "dismiss");
                                } else {
    ...
    if (panelConfig.type === "history") {
        ...
        // const historyAttrs = Object.values(assetInfo?.assetInfoDto?.asset?.attributes!).filter(item =>
        //     ["brightness", "colourRGB", "status"].includes(item.name)
        // );
        const historyAttrs = Object.values(assetInfo?.assetInfoDto?.asset?.attributes!).filter((attr) =>
        ...
        console.log('historyAttrs',Object.values(assetInfo?.assetInfoDto?.asset?.attributes!))
        ...
        const attributeChanged = (attributeName: string) => {
            if (hostElement.shadowRoot) {
                ...
                if (attributeName && attributeHistory) {
                    let attribute = asset.asset?.attributes && asset.asset?.attributes![attributeName];
                    const descriptors = AssetModelUtil.getAttributeAndValueDescriptors(asset.asset?.type, attribute!.name, attribute);
                    const label = Util.getAttributeLabel(attribute, descriptors[0], asset.asset?.type, true);
        ...
        return html`
            ...
            ${attrTemplate}
            <or-attribute-history id="attribute-history" .config="${viewerConfig.historyConfig}" .assetType="${asset.asset?.type}" .assetId="${asset.asset?.id}"></or-attribute-history>
        `;
    }

    if (panelConfig.type === "group") {

        if (!asset.asset?.id || asset.asset?.type !== "GroupAsset") {
            return;
        }
        ...
        const childAssetTypeAttribute = asset.asset?.attributes && asset.asset?.attributes[WellknownAttributes.CHILDASSETTYPE];
        ...
        const updateSelectedAttributes = (newSelection: string[]) => {
            ...
            config.views[asset.asset?.id!] = [...selectedAttributes];
        ...
        manager.console.retrieveData("OrAssetConfig")
            .then(conf => {
                ...
                const view = config.views[asset.asset?.id!];
        ...
        if ((!selectedAttributes || selectedAttributes.length === 0) && availableAttributes) {
            selectedAttributes = [...new Set(childAssets.map(childAsset => Object.keys(childAsset.asset?.attributes!)).flat())];
        }
        ...
        const getHeadersAndRows: () => [string[], string[][]] = () => {
            ...
            const headers = attrNames.map((attrName) => {
                ...
                return Util.getAttributeLabel(undefined, attributeDescriptor, asset.asset?.type, false);
            });
            ...
            const rows = childAssets.map((childAsset) => {
                ...
                const arr = attrNames.map((attributeName) => {
                    return childAsset.asset?.attributes![attributeName] ? childAsset.asset?.attributes![attributeName].value! as string : "";
                });
                arr.unshift(childAsset.asset!.name!);
                return arr;
            });
...
export function getAttributeTemplate(asset: AssetInfoDto, attribute: Attribute<any>, hostElement: LitElement, viewerConfig: AssetViewerConfig, panelConfig: PanelConfig, itemConfig: InfoPanelItemConfig): TemplateResult {
    ...
    return html`
        <or-attribute-input class="force-btn-padding" disablesubscribe .assetType="${asset!.asset?.type}" .attribute="${attribute}" .assetId="${asset.asset?.id!}" .disabled="${attrDisabled}" .label="${attrLabel}" .readonly="${attrReadonly}" resizeVertical .disableButton="${attrDisableButton}" .inputType="${attrInputType}" .hasHelperText="${!attrDisableHelper}" .fullWidth="${attribute.name === 'location' ? true : false}"></or-attribute-input>
    `;
}

export function getPropertyTemplate(asset:AssetInfoDto, property: string, hostElement: LitElement, viewerConfig: AssetViewerConfig | undefined, panelConfig: PanelConfig | undefined, itemConfig: InfoPanelItemConfig) {
    console.log('property',asset)
    let value = (asset as { [index: string]: any })[property];
    let value2 = (asset as { [index: string]: any })[property];
    ...
    switch (property) {
        ...
        case "cabinetId":
            // Display the path instead
            value = asset.assetInfo?.cabinetId;
            console.log('value2',value)
            // Populate value when we get the response
            const ancestors2 = [value];
            // Remove this asset from the path
            // ancestors2.pop();
            console.log('ancestors2',ancestors2)
            value = "";
            if (ancestors2.length > 0) {
                getAssetNames(ancestors2).then(
                    (names) => {
                        console.log('anem',names)
                        if (hostElement && hostElement.shadowRoot) {
                            const pathField = hostElement.shadowRoot.getElementById("property-cabinetId") as OrMwcInput;
                            if (pathField) {
                                pathField.value = names.join(" > ");
                            }
                            console.log('anem3',pathField?.value)
                        }
                    }
                );
                value2 = i18next.t("loading");
            }
            break;

        case "assetCode":
                value = asset.assetInfo?.assetCode
            break;
        case "cabinetAssetCode":
            value = asset.assetInfo?.cabinetAssetCode
            break;
        case "createdOn":
        ...
        case "accessPublicRead":
            ...
            value = asset.asset?.accessPublicRead
            break;
        case "isControlByCabinet":
            type = InputType.CHECKBOX;
            value = asset.assetInfo?.isControlByCabinet
            break;
        case "active":
            type = InputType.CHECKBOX;
            value = asset.assetInfo?.active
            break;
        case "type":
            value = asset.asset?.type
    }
    console.log('valueparent',value)
    console.log('valuecabinet',value)
    return html`<or-mwc-input check="${value}" id="property-${property}" .type="${type}" dense .value="${value}" .readonly="${itemConfig.readonly !== undefined ? itemConfig.readonly : true}" .label="${itemConfig.label}"></or-mwc-input>`;
}
...
async function getAssetNames(ids: string[]): Promise<string[]> {
    ...
    console.info('responseGetAssetName',response)
    if (response.status !== 200 || !response.data || response.data.length !== ids.length) {
        return ids;
    }

    return ids.map((id) => response.data.find((asset) => asset.asset?.id === id)!.asset?.name!);
}

async function getAssetChildren(parentId: string, childAssetType: string): Promise<AssetInfoDto[]> {
    ...
    try {
        ...
        console.log('getAssetChildren',response)
    } catch (e) {
    ...
    return response.data.filter((asset) => asset.asset?.type === childAssetType);
}
...
async function getLinkedUsers(asset: AssetInfoDto): Promise<UserAssetLinkInfo[]> {
    console.log('userAssetLinks',asset.asset?.id)

    try {
        return await manager.rest.api.AssetResource.getUserAssetLinks(
            {realm: manager.displayRealm, assetId: asset.asset?.id}
        ).then((response) => {
...
export async function saveAsset(asset: AssetInfoDto,childIds:any): Promise<SaveResult> {
    const isUpdate = !!asset.asset?.id && asset.asset?.version !== undefined;
    let success: boolean;
    let id: string = "";
    console.log('assetUpdate2',asset)
    try {
        if (isUpdate) {
            if (!asset.asset?.id) {
                throw new Error("Request to update existing asset but asset ID is not set");
            }
            console.log('assetUpdate2 truoc update',asset)
            console.log('list CHild',childIds)
            const response = await manager.rest.api.AssetResource.update(asset.asset?.id!, asset,{childIds});
            console.log('responseupdate',response)
            success = response.status === 200;
            id = asset.asset?.id!;
        } else {
            console.log('responseTruoccreate',asset)
...
export class OrAssetViewer extends subscribe(manager)(translate(i18next)(LitElement)) {
    ...
    constructor() {
        ...
        console.log('this._onAssetModified',this._onAssetModified)
    }
    ...
    shouldUpdate(changedProperties: PropertyValues): boolean {
        ...
        if (changedProperties.has("_assetInfo")) {
            console.log('sau change',this._assetInfo)
            console.log('sau change2',this.asset)
        }
        ...
        if (changedProperties.has("asset")) {
            ...
            if (this.asset) {
                this.loadAssetInfo(this.asset)
                    ...
                    .catch(reason => {
                        console.log('assetInfo',this._assetInfo)
    ...
    updated(_changedProperties: PropertyValues) {
        if (_changedProperties.has("asset")) {
            this._doValidation();
            console.log('assetID',this._assetInfo)
        }
    }

    async loadAssetInfo(asset: AssetInfoDto): Promise<AssetInfo> {

        if (!asset) {
            throw new Error("Asset has changed");
        }
        console.log('aaasss',asset)
        if (!asset.asset?.attributes) {
            asset.asset!.attributes = {};
        }
        console.log('aaasss',asset.asset!.attributes)

        const exists = !!asset.asset?.id;
        const modified = !exists;
        const viewerConfig = this._getPanelConfig(asset);
        console.log('exit',exists)
        console.log('modi',modified)
        console.log('viewconfig',viewerConfig)
        if (!exists) {
        ...
        if (!this.ids || this.ids.length != 1 || this.ids[0] !== asset.asset?.id) {
            throw new Error("Asset has changed");
        }

        // Load child assets for group asset
        let childAssets: AssetInfoDto[] | undefined = undefined;
        if (asset.asset?.type === WellknownAssets.GROUPASSET) {
            childAssets = await getAssetChildren(asset.asset?.id!, asset.asset?.attributes![WellknownAttributes.CHILDASSETTYPE].value);
    ...
    protected _doValidation() {
        if (this.editMode && this.editor) {
            ...
            console.log('_validationResults',this._validationResults)
        }
    }
    ...
    async reloadBUIE() {
        console.log('thâ',this._assetInfo)
        try {
            const response = await manager.rest.api.AssetResource.reloadBUIE(this._assetInfo?.assetInfoDto?.asset?.id);
        } catch (e) {
            console.log('call Faile')
        }
    }
    protected render(): TemplateResult | void {
        console.log('abcd',this.childIds)
        ...
        console.log('Hien thi tren giao dien',this._assetInfo)
        if (!this._assetInfo) {
            return html`
        ...
        if (!this._assetInfo.assetInfoDto) {
            return html`
                <div><or-translate value="notFound"></or-translate></div>
            `;
        }

        const asset = this._assetInfo.assetInfoDto;
        console.log('Info',this.asset)
        const descriptor = AssetModelUtil.getAssetDescriptor(asset.asset?.type!);
                ...
                content = html`
                    <div id="view-container"
                        ...
                        <div id="left-column" class="panelContainer">
                           <div class=" panel mobileHidden " style="" id="1-panel">
                                <div class="panel-content-wrapper">
                                    <div class="panel-title" style="display:flex;justify-content: space-between;align-items: center">
                                        <or-translate value="information"></or-translate>
                                        <or-icon class="beauty-button" icon="refresh" style="color:green;cursor: pointer" @click="${this.reloadBUIE}"></or-icon>
                                    </div>
                                    <div class="panel-content">
                                        <div id="field-notes" style="" class=" field ">
                                            <div style="display: flex;flex-direction: column">
                                                <label>${i18next.t("assetCode")}</label>
                                                <or-mwc-input id="parent" .type="${InputType.TEXT}"
                                                              readonly
                                                              value="${this._assetInfo.assetInfoDto.assetInfo?.assetCode}"
                                                              .label="${i18next.t("assetCode")}"
                                                >
                                                    ></or-mwc-input>
                                                <label>${i18next.t("cabinetAssetCode")}</label>
                                                <or-mwc-input id="parent" .type="${InputType.TEXT}"
                                                              readonly
                                                              value="${this._assetInfo.assetInfoDto.assetInfo?.cabinetAssetCode}"
                                                              .label="${i18next.t("cabinetAssetCode")}"
                                                >
                                                    ></or-mwc-input>
                                                <label>${i18next.t("cabinetId")}</label>
                                                <or-mwc-input id="parent" .type="${InputType.TEXT}"
                                                              .label="${i18next.t("cabinetId")}"  readonly
                                                              value="${this._assetInfo.assetInfoDto.assetInfo?.cabinetId}"
                                                ></or-mwc-input>
                                                <label>${i18next.t("isControlByCabinet")}</label>
                                                <div class="mdc-switch__thumb-underlay">
                                                    <div class="mdc-switch__thumb">
                                                        <input type="checkbox" id="elem"
                                                               disabled
                                                               checked="${this._assetInfo.assetInfoDto.assetInfo?.isControlByCabinet}"
                                                               class="mdc-switch__native-control"
                                                               role="switch">
                                                    </div>
                                                </div>
                                                <label>${i18next.t("Active")}</label>
                                                <div class="mdc-switch__thumb">
                                                    <input type="checkbox" id="elem" class="mdc-switch__native-control"
                                                           role="switch"
                                                           disabled
                                                           checked="${this._assetInfo.assetInfoDto.assetInfo?.active}"
                                                    >
            ...
            <div id="wrapper">
                <div id="asset-header" class=${editMode ? "editmode" : ""}>
                    ...
                    <div id="title">
                        ...
                        ${editMode
                                ? html`
                                    <or-mwc-input id="name-input" .type="${InputType.TEXT}" min="1" max="1023" comfortable required outlined .label="${i18next.t("name")}" .value="${asset.asset?.name}" @or-mwc-input-changed="${(e: OrInputChangedEvent) => {asset!.asset!.name = e.detail.value; this._assetInfo!.modified = true; this._doValidation();}}"></or-mwc-input>
                                `
                                : html`<span>${asset.asset?.name}</span>`}
                    </div>
                    <div id="right-wrapper" class="mobileHidden">
                        ${validationErrors.length === 0 ? (asset!.asset?.createdOn ? html`<or-translate id="created-time" class="tabletHidden" value="createdOnWithDate" .options="${{ date: new Date(asset!.asset?.createdOn!) } as TOptions<InitOptions>}"></or-translate>` : ``) : html`<span id="error-wrapper" .title="${validationErrors.join("\n")}"><or-icon icon="alert"></or-icon><or-translate class="tabletHidden" value="validation.invalidAsset"></or-translate></span>`}
                        ...
                        ${!this._isReadonly() ? html`<or-mwc-input id="edit-btn" .disabled="${!this._assetInfo.assetInfoDto.asset?.id}" outlined .type="${InputType.BUTTON}" .value="${this.editMode}" .label="${this.editMode ? i18next.t("viewAsset") : i18next.t("editAsset")}" icon="${this.editMode ? "eye" : "pencil"}" @or-mwc-input-changed="${() => this._onEditToggleClicked(!this.editMode!)}"></or-mwc-input>
                        `: ``}
                    </div>
                </div>
                ${content}
    ...
    protected _onSaveClicked() {
        if (!this._assetInfo || !this._assetInfo.assetInfoDto) {
            return;
        }
         console.log('clickSave',this._assetInfo)
        Util.dispatchCancellableEvent(this, new OrAssetViewerRequestSaveEvent(this._assetInfo.assetInfoDto))
            .then((detail) => {
                if (detail.allow) {
                    this._doSave();
                }
            });
        // if (this._assetInfo?.assetInfoDto?.assetInfo) {
    ...
    protected async _doSave() {
        ...
        const assetInfoDto = this._assetInfo.assetInfoDto;
        console.log('a22', this.saveBtnElem)
        this.saveBtnElem.disabled = true;
        if( this.wrapperElem){
            this.wrapperElem.classList.add("saving");
        }

        console.log('a23', this.wrapperElem)
        this._saveResult = await saveAsset(assetInfoDto,this.childIds);
        console.log('_saveResult',this._saveResult)
        if( this.wrapperElem){
            this.wrapperElem.classList.remove("saving");
        }

        console.log('a24', this.wrapperElem)
        if(this.saveBtnElem){
            this.saveBtnElem.disabled = false;
        }

        if (!this._assetInfo || this._assetInfo.assetInfoDto !== assetInfoDto) {
            // Asset has changed during save so ignore save result
            return;
        }

        if (this._saveResult.success) {
            console.log('success',this._saveResult)
            this._assetInfo.modified = false;
            this.asset = undefined;
            this.ids = [this._saveResult.assetId];
        }
       console.log('mkl',this._assetInfo)
    ...
    _onEvent(event: SharedEvent) {
        ...
        const processEvent = (event.eventType === "asset" && (event as AssetEvent).assetInfoDto?.asset?.id === assetId) || (event.eventType === "attribute" && (event as AttributeEvent).ref!.id == assetId);
        const assetTest23 = (event as AssetEvent)
        console.log('processevent',event)
        console.log('assetId',assetTest23)
        if (!processEvent) {
            return;
        }
        console.log('assetID',event)
        if (event.eventType === "asset") {
            console.log('assetTest',event)
            const asset = (event as AssetEvent).assetInfoDto!;
            console.log('dtoo',asset)
            const assetTest = (event as AssetEvent);
            if (!this._assetInfo) {
            ...
            if (this.editMode) {
                ...
                console.log('editthis._assetInfo',this._assetInfo)
                console.log('editthis._assetInfo',this._saveResult)
                ...
                console.log('this._assetInfo a',this._assetInfo )
    ...
    protected _getPanelConfig(asset: AssetInfoDto): AssetViewerConfig {
        ...
        if (this.config) {
            ...
            const assetConfig = this.config.assetTypes && this.config.assetTypes.hasOwnProperty(asset.asset?.type!) ? this.config.assetTypes[asset.asset?.type!] : this.config.default;
