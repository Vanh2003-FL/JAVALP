...
export function getAttributeValueAsString(attribute: Attribute<any> |undefined, descriptor: AttributeDescriptor | string | undefined, assetType: string | undefined, showUnits: boolean, fallback?: string): string {
 ...
 */
export function updateAsset(asset: AssetInfoDto, event: AttributeEvent): AssetInfoDto {

    const attributeName = event.ref!.name!;

    if (asset.asset?.attributes) {
        if (event.deleted) {
            delete asset.asset?.attributes![attributeName];
        } else {
            const attribute = asset.asset?.attributes[attributeName];
            if (attribute) {
